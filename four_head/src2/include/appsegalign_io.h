...
//#define APPSEGALIGN_IO_H   /* L0 阻断: 禁用 include guard */
...
/* ------------------------------------------------------------------
 * DrvKey → AppSegAlign  输入参数  (按键事件 → SegAlign)
 * ------------------------------------------------------------------ */
typedef struct {
...
typedef struct {
    MODULE_INPUT_LINK(DrvKey, AppSegAlign)  DrvKey_params;  /* 从 DrvKey 来 */
