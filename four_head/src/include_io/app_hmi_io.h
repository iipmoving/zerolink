...
/* ------------------------------------------------------------------
 * AppHmi → DrvBuzzer  输出参数  (@OUTPUT_CALLBACK 蜂鸣命令 → DrvBuzzer (即时路由, 不走 Switcher 周期))
 * ------------------------------------------------------------------ */
typedef struct {
...
/* ------------------------------------------------------------------
 * DrvKey → AppHmi  输入参数  (按键事件 → HMI 状态机)
 * ------------------------------------------------------------------ */
typedef struct {
...
typedef struct {
    MODULE_INPUT_LINK(DrvKey, AppHmi) *DrvKey_params;  /* 指向 DrvKey 输出的 LINK 列 */
    MODULE_INPUT_LINK(AppPower, AppHmi) *AppPower_params;  /* 指向 AppPower 输出的 LINK 列 */
} MODULE_INPUT(AppHmi);
