...
//#define APPCOOKING_IO_H   /* L0 阻断: 禁用 include guard */
...
/* ------------------------------------------------------------------
 * DrvKey → AppCooking  输入参数  (按键事件 → Cooking)
 * ------------------------------------------------------------------ */
typedef struct {
...
typedef struct {
    MODULE_INPUT_LINK(DrvKey, AppCooking)  DrvKey_params;  /* 从 DrvKey 来 */
    MODULE_INPUT_LINK(AppCommMgr, AppCooking)  AppCommMgr_params;  /* 从 AppCommMgr 来 */
