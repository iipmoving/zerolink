...
typedef struct
{
  ...
  uint32_t Tx_msg[16]; /*!< This member configures the sending data of CAN*/
	...
	uint8_t Padding;  /*!< Add 1 bytes of padding to avoid the -Wpadded warning. */
}CanTxMsg;
...
ErrorStatus CAN_SBaudRate_Select(CAN_TypeDef *CANx,uint32_t CAN_Clk,uint8_t CAN_FPrescaler,uint32_t BaudRate);
