...
#endif
#if defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806) || defined(SC32L14xx) || defined(SC32R803)|| defined(SC32f15xx)||defined(SC32R601)||defined(SC32R807)
/**
        ...
        if ( ( RCC->RCC_CFG0 & RCC_CFG0_SYSCLKSW ) != RESET )
				{
					...
					for ( uint32_t i = 1; i <= ((RCC->PLL_CFG &0x03) + 1 ); i++ )
					{
							Multioperator *=  2;
					}
...
void RCC_GetClocksFreq ( RCC_ClocksTypeDef* RCC_Clocks )
{
    ...
    if ( ( RCC->RCC_CFG0 & RCC_CFG0_SYSCLKSW ) == RESET )
    {
#if defined(SC32f10xx) || defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806) || defined(SC32L14xx) || defined(SC32R803) || defined(SC32f15xx)|| defined(SC32R601)||defined(SC32R807)
    ...
    else
    {
        ...
        case RCC_SYSCLKSource_PLLRCLK:   /* PLL used as system clock source */
            if ( ( RCC->PLL_CFG & PLL_CFG_PLLCLKSEL ) == RESET ) /* HIRC used as PLL clock source */
            {
                pllsource = HIRC_VALUE;
            }
            else   /* HXT used as PLL clock source */
            {
                pllsource = HXT_VALUE;
            }
...
void RCC_PWM0CLKConfig ( RCC_PWM0CLKSource_TypeDef RCC_PWM0CLKSource )
{
    ...
    RCC->RCC_CFG1 |= ( uint32_t ) ( RCC_PWM0CLKSource );
		if(RCC_PWM0CLKSource != RCC_PWM0CLKSource_PCLK)
		{
				 while(!(RCC->RCC_CFG1&RCC_CFG1_PWM0CLKSEL))
				{
					delay_time--;
					if(delay_time == 0)
					{
...
void RCC_LCDLEDCLKConfig ( RCC_LCDLEDCLKSource_TypeDef RCC_LCDLEDCLKSource )
{
    ...
    assert_param ( RCC_LCDLEDCLKSOURCE ( RCC_LCDLEDCLKSource ) );
		...
		if(RCC_LCDLEDCLKSource == RCC_LCDLEDCLKSource_LXT)
		{
				RCC->RCC_CFG0 &= ~ ( ( uint32_t ) RCC_CFG0_LXTSYSEN );
				RCC->RCC_CFG0 |= ( uint32_t ) ( RCC_LXTSYSEN_Enable );
		}
		else
		{
				RCC->RCC_CFG0 &= ~ ( ( uint32_t ) RCC_CFG0_LXTSYSEN );
				RCC->RCC_CFG0 |= ( uint32_t ) ( RCC_LXTSYSEN_Disable );
		}
#endif
    ...
    RCC->RCC_CFG1 |= ( uint32_t ) ( RCC_LCDLEDCLKSource );
		if(RCC_LCDLEDCLKSource == RCC_LCDLEDCLKSource_LXT)
		{
				 while(!(RCC->RCC_CFG1&RCC_CFG1_LCDCLKSEL))
				{
					delay_time--;
					if(delay_time == 0)
					{
...
void RCC_BTMCLKConfig ( RCC_BTMCLKSource_TypeDef RCC_BTMCLKSource )
{
...
#if defined(SC32L14xx)||defined(SC32R807)
		if(RCC_BTMCLKSource == RCC_BTMCLKSource_LXT)
		{
				RCC->RCC_CFG0 &= ~ ( ( uint32_t ) RCC_CFG0_LXTSYSEN );
				RCC->RCC_CFG0 |= ( uint32_t ) ( RCC_LXTSYSEN_Enable );
		}
    ...
    RCC->RCC_CFG1 |= ( uint32_t ) ( RCC_BTMCLKSource );
		if(RCC_BTMCLKSource == RCC_BTMCLKSource_LXT)
		{
				 while(!(RCC->RCC_CFG1&RCC_CFG1_BTMCLKSEL))
				{
					delay_time--;
					if(delay_time == 0)
					{
...
#if defined(SC32L14xx)||defined(SC32R807)
				if(RCC_SysTickSource == RCC_SysTickSource_LXT)
				{
						RCC->RCC_CFG0 &= ~ ( ( uint32_t ) RCC_CFG0_LXTSYSEN );
						RCC->RCC_CFG0 |= ( uint32_t ) ( RCC_LXTSYSEN_Enable );
				}
