...
void SPI_Init ( SPI_TypeDef* SPIx, SPI_InitTypeDef* SPI_InitStruct )
{
    ...
    /*---------------------------- SPIx SPI0_CON Configuration ------------------------*/
		...
		tmpreg = SPIx->SPI_CON;
		/* Clear MSTR, SPMD, DORD, CPOL, SPR bits */
		tmpreg &= ( uint32_t ) ~ ( SPI_CON_MSTR | SPI_CON_DWIDTH | SPI_CON_DORD |
                               SPI_CON_CPHA | SPI_CON_CPOL | SPI_CON_QTWCK |SPI_CON_MODE );
		/* Configure SPIx: mode, data size, first transmitted bit,clock predivision , CPOL and CPHA */
		/* Set MSTR bits to SPI0_Mode value */
		/* Set SPMD bit according to SPI1_DataSize value */
		/* Set DORD bit according to SPI1_FirstBit value */
		...
		tmpreg |= ( uint32_t ) ( SPI_InitStruct->SPI_Mode | SPI_InitStruct->SPI_DataSize |
																 SPI_InitStruct->SPI_FirstBit | SPI_InitStruct->SPI_CPHA |
																 SPI_InitStruct->SPI_CPOL | SPI_InitStruct->SPI_Prescaler|(0X1 <<SPI_CON_MODE_Pos));
...
void SPI_Cmd ( SPI_TypeDef* SPIx, FunctionalState NewState )
{
		...
		/* If it is SPI0 and mapped as SPI_PinRemap_B, disable the SPI */
		if((SPIx == SPI0) && ((SPIx->SPI_CON & SPI_CON_SPOS) == SPI_PinRemap_B))
		{
				NewState = DISABLE;
		}
		/* If it is SPI1 and mapped as SPI_PinRemap_B, disable the SPI */
		if((SPIx == SPI1) && ((SPIx->SPI_CON & SPI_CON_SPOS) == SPI_PinRemap_B))
		{
				NewState = DISABLE;
		}
#endif
...
void SPI_DataSizeConfig ( SPI_TypeDef* SPIx, SPI_DataSize_TypeDef SPI_DataSize )
{
    ...
    assert_param ( IS_SPI_DATASIZE ( SPI_DataSize ) );
		...
		/* Clear SPMD bit */
		 SPIx->SPI_CON &= ~TWI_QSPIx_CON_DWIDTH;
		/* Set new SPMD bit value */
		 SPIx->SPI_CON |= SPI_DataSize ;		
#elif defined(SC32L14xx)||defined(SC32R807)
    /* Check the parameters */
...
void SPI_ClearFlag ( SPI_TypeDef* SPIx, uint32_t SPI_FLAG )
{
	...
	assert_param ( IS_SPI_ALL_PERIPH ( SPIx ) );
	/* Clear the flags */
	tmpreg = SPIx->SPI_STS & 0xFFFF0000U;
	tmpreg |= SPI_FLAG;
	SPIx->SPI_STS = tmpreg;
#elif defined(SC32f15xx)||defined(SC32R601)
  /* Check the parameters */
