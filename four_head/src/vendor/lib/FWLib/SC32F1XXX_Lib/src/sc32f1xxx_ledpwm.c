...
void LEDPWM_Init ( LEDPWM_InitTypeDef* LEDPWM_InitStruct )
{
    uint32_t tmpreg;
		...
		/* The SC32F12xx has no LEDPWM7 channel for LEDPWM */
		if((LEDPWM_InitStruct->LEDPWM_OutputChannel & LEDPWM_Channel_7) == LEDPWM_Channel_7)
		{
				LEDPWM_InitStruct->LEDPWM_OutputChannel &= ~(uint32_t)LEDPWM_Channel_7;
		}
#endif
#endif
    ...
    LEDPWM->LEDPWM_CHN = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_OutputChannel );

    /* Write to LEDPWM LEDPWM_INV */
    LEDPWM->LEDPWM_INV = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_LowPolarityChannl );
#elif defined(SC32R803)
		/* Write to LEDPWM LEDPWM_CHN0 */
		LEDPWM->LEDPWM_CHN0 = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_OutputChannel0 );
		/* Write to LEDPWM LEDPWM_INV0 */
		LEDPWM->LEDPWM_INV0 = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_LowPolarityChannl0 );

    if ( LEDPWM_InitStruct->LEDPWM_OutputChannel1 <=(uint32_t) LEDPWM_Channel_32_38 )
    {
				/* Write to LEDPWM LEDPWM_CHN1 */
        LEDPWM->LEDPWM_CHN1 = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_OutputChannel1 & 0x7FFFFFFF );
				/* Write to LEDPWM LEDPWM_INV1 */
        LEDPWM->LEDPWM_INV1 = ( uint32_t )( LEDPWM_InitStruct->LEDPWM_LowPolarityChannl1 & 0x7FFFFFFF );
    }
#endif
...
void LEDPWM_SetDuty ( LEDPWM_Channel_Typedef LEDPWM_Channel, uint8_t LEDPWM_Duty )
{
...
#elif defined (SC32R803)
    if ( (uint32_t)LEDPWM_Channel <= 0x80000000 )
    {
        for ( tmpvalue = 0; tmpvalue < 32; tmpvalue++ )
        {
            if ( ( uint32_t ) LEDPWM_Channel & tmpchannel )
    ...
    else
    {
        for ( tmpvalue = 0; tmpvalue < 7; tmpvalue++ )
        {
            if ( ( uint32_t ) LEDPWM_Channel & tmpchannel )
            {
                LEDPWM->LEDPWM_DT[tmpvalue + 32] = LEDPWM_Duty;
            }
            tmpchannel = tmpchannel << 1;
        }
    }
...
uint8_t LEDPWM_GetDuty ( LEDPWM_Channel_Typedef LEDPWM_Channel )
{
    ...
    for ( tmpvalue = 0; tmpvalue < 32; tmpvalue++ )
    {
        if ( ( uint32_t ) LEDPWM_Channel & tmpchannel )
        {
            return ( uint8_t ) ( LEDPWM->LEDPWM_DT[tmpvalue] );
        }
        tmpchannel = tmpchannel << 1;
    }
#elif defined (SC32R803)
    ...
    else
    {
        ...
        for ( tmpvalue = 0; tmpvalue < 7; tmpvalue++ )
        {
            if ( ( uint32_t ) LEDPWM_Channel & tmpchannel )
            {
                return ( uint8_t ) ( LEDPWM->LEDPWM_DT[tmpvalue + 32] );
            }
            tmpchannel = tmpchannel << 1;
        }
    }
