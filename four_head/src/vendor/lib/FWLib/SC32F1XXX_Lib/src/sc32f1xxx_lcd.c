...
void LCD_Init ( LCD_InitTypeDef* LCD_InitStruct )
{
    ...
    assert_param ( IS_LCD_DUTY ( LCD_InitStruct->LCD_Duty ) );
		...
		/* The SC32F12xx has no SEG7 channel for LCD */
		if((LCD_InitStruct->LCD_SegPin & LCD_Channel_7) == LCD_Channel_7)
		{
				LCD_InitStruct->LCD_SegPin &= ~(uint32_t)LCD_Channel_7;
		}
#endif
#endif
    ...
    tmpreg &= ( uint32_t ) ~ ( DDR_CFG_VOIRS | DDR_CFG_DUTY  | DDR_CFG_SCS );
...
void LCD_SEGConfig ( uint64_t SEGSelect, FunctionalState NewState )
{
...
#if defined(SC32f12xx)
		/* The SC32F12xx has no SEG7 channel for LCD */
		if((SEGSelect & LCD_Channel_7) == LCD_Channel_7)
		{
				SEGSelect &= ~(uint32_t)LCD_Channel_7;
		}
#endif
#endif
    ...
    if ( NewState == ENABLE )
    {
        temp |= (uint32_t)SEGSelect;
    }
    else
    {
        temp &= ~(uint32_t)SEGSelect;
    }
    LCD_LED->SEG_EN = (uint32_t)SEGSelect;
    ...
    if ( NewState == ENABLE )
    {
        temp0 |= (uint32_t)SEGSelect;
        temp1 |= (uint32_t)(SEGSelect >> 32);
    }
    else
    {
        temp0 &= ~(uint32_t)SEGSelect;
...
{
    /* Enable LCD reset state */
    RCC_APB2PeriphResetCmd ( RCC_APB2Periph_LCD, ENABLE );
    /* Disable LCD reset state */
    RCC_APB2PeriphResetCmd ( RCC_APB2Periph_LCD, DISABLE );
}
...
void LCD_Init ( LCD_InitTypeDef* LCD_InitStruct )
{
    ...
    /* Clear DMOD,BIAS,VOIRSIF,TYPE,DDRCK and TRIMODE bits */
    tmpreg &= ( uint32_t ) ~ ( DDR_CON_BIAS | DDR_CON_VOIRSF | DDR_CON_TYPE | DDR_CON_DDRON | DDR_CON_DDRCK \
															 | DDR_CON_TRICOM | DDR_CON_TRIMODE );
    ...
    /* Write to LCD_LEDx SEG_EN */
    LCD->SEG_EN0 = ( uint32_t ) LCD_InitStruct->LCD_SegPin;
    LCD->SEG_EN1 = ( uint32_t ) ( LCD_InitStruct->LCD_SegPin >> 32 );

    /* Write to LCD_LEDx COM_EN */
    LCD->COM_EN = LCD_InitStruct->LCD_ComPin;
}
...
void LCD_StructInit ( LCD_InitTypeDef* LCD_InitStruct )
{
    ...
    LCD_InitStruct->LCD_Duty = LCD_Duty_1_8;
    LCD_InitStruct->LCD_FrameFre = LCD_FrameFre_B64Hz;
    LCD_InitStruct->LCD_ResSel = LCD_ResSel_11K;
    LCD_InitStruct->LCD_SegPin =  LCD_Channel_Less;
    LCD_InitStruct->LCD_VOIRSIF = LCD_VOIRSIF_Disable;
    LCD_InitStruct->LCD_Voltage = 0;
}
...
void LCD_Cmd ( FunctionalState NewState )
{
    ...
    {
        /* Enable the LCD_LED Counter */
        LCD->DDR_CON |= DDR_CON_DDRON;
    }
    else
    {
        /* Disable the LCD_LED Counter */
        LCD->DDR_CON &= ( uint16_t ) ~DDR_CON_DDRON;
    }
}
...
void LCD_SEGConfig ( uint64_t SEGSelect, FunctionalState NewState )
{
    ...
    LCD->SEG_EN0 = ( uint32_t ) temp0;
    LCD->SEG_EN1 = ( uint32_t ) temp1;
}
/**
 * @brief  Writes data to SEG RAM.
 * @param  LCD_RAMRegister[in]:LCD RAM register selection.
...
void LCD_Write ( LCD_RAMRegister_Typedef LCD_RAMRegister, uint8_t LCD_LED_Data )
{
    LCD->SEGRn[LCD_RAMRegister] = LCD_LED_Data;
}
...
void LCD_CustomModeScan ( void )
{
    LCD->DDR_CON |= DDR_CON_TRICOM;
}

/**
  * @brief  Capacitive LCD Configuration and PUMP Clock Division Settings.
  * @param  LCD_DriveMode[out]: Pointer to structure LCD_DriveMode_Typedef, to be initialized.
  * @retval None
  */
void LCD_CapacitiveDriveMode ( LCD_PUMPClockDiv_Typedef LCD_PUMPCKDiv )
{
		/* Allow LCD PUMP */
		LCD->DDR_CON |= 0x00000001U << DDR_CON_PUMPON_Pos;
	
		/* Select a capacitive-type LCD driver */
		LCD->DDR_CON |= 0x00000001U << DDR_CON_LCDSEL_Pos;
	
		/* PUMP clock frequency setting */
    LCD->DDR_CON &= ~ (0x00000001U << DDR_CON_PUMP_Pos);
		LCD->DDR_CON |= LCD_PUMPCKDiv << DDR_CON_PUMP_Pos;
}
