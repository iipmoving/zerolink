...
void UART_TXCmd ( UART_TypeDef* UARTx, FunctionalState NewState )
{
    ...
    assert_param ( IS_FUNCTIONAL_STATE ( NewState ) );
		...
		if((UARTx == UART0) && ((UART0->UART_CON & UART_CON_SPOS) == 0))
		{
				PC_BIT(8) = 1;
		}
		...
		if((UARTx == UART3) && ((UART3->UART_CON & UART_CON_SPOS) == 0))
		{
				PE_BIT(7) = 1;
		}
#endif
...
void UART_RXCmd ( UART_TypeDef* UARTx, FunctionalState NewState )
{
    ...
    assert_param ( IS_FUNCTIONAL_STATE ( NewState ) );
		...
		{
				PC_BIT(7) = 0;
				GPIOC->PXCON |= ( uint32_t )0x00000080;
		}
#elif defined(SC32R803)
		if((UARTx == UART1) && ((UART1->UART_CON & UART_CON_SPOS) == 0))
		{
				PD_BIT(4) = 0;
				GPIOD->PXCON |= ( uint32_t )0x00000010;
		}
#elif defined(SC32L14xx)||defined(SC32R807)
		if((UARTx == UART3) && ((UART3->UART_CON & UART_CON_SPOS) == 0))
		{
				PE_BIT(6) = 0;
				GPIOE->PXCON |= ( uint32_t )0x00000040;
		}
#endif
#endif
...
/**
 * @brief  Enables or disables the data matching.
 * @param  UARTx[out]:  where x can be to select the UARTx peripheral.
 *                   SC32f10xx Selection range(UART0 - UART3)
 *                   SC32R803 Selection range(UART0 - UART5)
 *                   SC32f12xx Selection range(UART0 - UART5)
...
void UART_DataMatchCmd ( UART_TypeDef* UARTx, FunctionalState NewState )
{
    /* Check the parameters */
    assert_param ( IS_UART_ALL_PERIPH ( UARTx ) );
    assert_param ( IS_FUNCTIONAL_STATE ( NewState ) );
...
void UART_SendData ( UART_TypeDef* UARTx, uint16_t Data )
{
    ...
    assert_param ( IS_UART_ALL_PERIPH ( UARTx ) );
		...
		{
				/* Save the RX status */
				RXStatus = UARTx->UART_CON & ( uint32_t ) UART_CON_RXEN;
				/* Disable the UART RX Function */
        UARTx->UART_CON &= ~UART_CON_RXEN;
		}
#elif defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806)
...
#elif defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806) || defined(SC32L14xx) ||defined(SC32f15xx)||defined(SC32R601)||defined(SC32R807)
    tmpreg = UARTx->UART_CON;
...
/**
 * @brief  UART preset data matching for received data configuration.
 * @param  UARTx[out]:  where x can be to select the UARTx peripheral.
 *                  - UART0:UART peripheral select UART0
 *                  - UART1:UART peripheral select UART1
 *                  - UART2:UART peripheral select UART2
 ...
 *                  - UART4:UART peripheral select UART4
 *                  - UART5:UART peripheral select UART5
 * @param  PRESET_DATA[in]: UART preset data matching for received data.
 * @retval None
 */
#if defined(SC32L14xx)||defined(SC32R807)
void UART_MatchDataConfig ( UART_TypeDef* UARTx, uint16_t PRESET_DATA )
{
    /* Check the parameters */
    assert_param ( IS_UART_ALL_PERIPH ( UARTx ) );

		/* Preset data matching */
		UARTx->UART_DMR = ( PRESET_DATA & ( uint16_t ) 0x01FF );
}
#endif

/* End of UART_Group3.	*/
#if defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806) || defined(SC32L14xx)|| defined(SC32f15xx)|| defined(SC32R803)|| defined(SC32R601)||defined(SC32R807)
...
uint8_t LIN_CalID ( uint8_t id )
{
    ...
    parity = id;
    p0 = (uint8_t)(( BIT ( parity, 0 ) ^ BIT ( parity, 1 ) ^ BIT ( parity, 2 ) ^ BIT ( parity, 4 ) ) << 6);
    p1 = (uint8_t)(( ! ( BIT ( parity, 1 ) ^ BIT ( parity, 3 ) ^ BIT ( parity, 4 ) ^ BIT ( parity, 5 ) ) ) << 7);
...
static UART_TypeDef* Printf_Uart;
#if defined (__ARMCC_VERSION) && (__ARMCC_VERSION >= 6010050)
__asm(".global __use_no_semihosting\n\t");
#elif defined(__ARMCC_VERSION)
// Disable semihosting to avoid host dependency 
...
FILE  __stdout;
// Redefine _sys_exit() to avoid semihosting mode
