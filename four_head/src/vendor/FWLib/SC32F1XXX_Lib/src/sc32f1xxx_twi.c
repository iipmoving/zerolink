...
void TWI_Init ( TWI_TypeDef* TWIx, TWI_InitTypeDef* TWI_InitStruct )
{
  ...
  uint32_t tmpreg;
    ...
    tmpreg = TWIx->TWI_CON;
    /* Clear AA, TWCK, STRETCH bits */
    tmpreg &= ( uint32_t ) ~ ( TWI_CON_AA | TWI_CON_SPOS | TWI_CON_STRETCH |TWI_CON_MODE);
    /* Configure TWIx: Ack, Clock division and Slave Clock Stretch */
    /* Set AA bits to TWI_Ack value */
    /* Set TWCK bits according to TWI_DataSize value */
    /* Set STRETCH bit according to TWI_Stretch value */
  ...
  if(TWIx == TWI1)
	{
		tmpreg &=~(uint32_t)(0x00<<SPI1_TWI1_CON_MODE_Pos);
	}
  /* Write to TWIx TWI_CON */
  TWIx->TWI_CON = tmpreg;
  /*---------------------------- TWIx TWI_ADD Configuration ------------------------*/
...
void TWI_Cmd ( TWI_TypeDef* TWIx, FunctionalState NewState )
{
		...
		/* If it is TWI0 and mapped as TWI_PinRemap_E, disable the TWI */
		if((TWIx == TWI0) && ((TWIx->TWI_CON & TWI_CON_SPOS) == TWI_PinRemap_E))
		{
				NewState = DISABLE;
		}
		/* If it is TWI1 and mapped as TWI_PinRemap_Default, disable the TWI */
		if((TWIx == TWI1) && ((TWIx->TWI_CON & TWI_CON_SPOS) == TWI_PinRemap_Default))
		{
				NewState = DISABLE;
		}
#endif
