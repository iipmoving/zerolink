...
void LED_Init ( LED_InitTypeDef* LED_InitStruct )
{
    ...
    assert_param ( IS_LED_DUTY ( LED_InitStruct->LED_Duty ) );
		...
		/* The SC32F12xx has no SEG7 channel for LED */
		if((LED_InitStruct->LED_SegPin & LED_Channel_7) == LED_Channel_7)
		{
				LED_InitStruct->LED_SegPin &= ~(uint32_t)LED_Channel_7;
		}
#endif
#endif
...
#if defined(SC32f12xx)
		/* The SC32F12xx has no SEG7 channel for LED */
		if((SEGSelect & LED_Channel_7) == LED_Channel_7)
		{
				SEGSelect &= ~(uint32_t)LED_Channel_7;
		}
#endif
#endif
