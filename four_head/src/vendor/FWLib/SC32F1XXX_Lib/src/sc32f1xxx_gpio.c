 ...
 *                SC32L14xx Selection range(GPIOA - GPIOE)
 *                SC32R807  Selection range(GPIOA - GPIOE)
 ...
 *               - GPIOE: select the GPIOE peripheral
 * @retval None
 */
...
void GPIO_Init ( GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct )
{
    ...
    assert_param ( IS_GPIO_MODE ( GPIO_InitStruct->GPIO_Mode ) );
		...
		/* If the currently initialized GPIO is the TX pin of the programming port UART, 
	     forcibly set the GPIO mode to strong push-pull output, ResetBit the GPIO bit 
			 of the tDIO pin on the programming port. */
#if (UARTOptSwitch)
#if defined(SC32f10xx)
		if((GPIOx == GPIOC) && ((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_3) == GPIO_Pin_3))
		{
				GPIO_InitStruct->GPIO_Pin &= ~GPIO_Pin_3;
				PC_BIT(3) = 0;
				GPIOx->PXCON |= ( uint32_t ) GPIO_Pin_3;
		}
#elif defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806)
		if((GPIOx == GPIOB) && ((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_1) == GPIO_Pin_1))
		{
				GPIO_InitStruct->GPIO_Pin &= ~GPIO_Pin_1;
				PB_BIT(1) = 0;
				GPIOx->PXCON |= ( uint32_t ) GPIO_Pin_1;
		}
#elif defined(SC32f15xx) || defined(SC32R601)
		if((GPIOx == GPIOC) && ((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_8) == GPIO_Pin_8))
		{
				GPIO_InitStruct->GPIO_Pin &= ~GPIO_Pin_8;
				PC_BIT(8) = 0;
				GPIOx->PXCON |= ( uint32_t ) GPIO_Pin_8;
		}
#elif defined(SC32R803)
		if((GPIOx == GPIOD) && ((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_5) == GPIO_Pin_5))
		{
				GPIO_InitStruct->GPIO_Pin &= ~GPIO_Pin_5;
				PD_BIT(5) = 0;
				GPIOx->PXCON |= ( uint32_t ) GPIO_Pin_5;
		}
#elif defined(SC32L14xx)||defined(SC32R807)
		if((GPIOx == GPIOE) && ((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_7) == GPIO_Pin_7))
		{
				GPIO_InitStruct->GPIO_Pin &= ~GPIO_Pin_7;
				PE_BIT(7) = 0;
				GPIOx->PXCON |= ( uint32_t ) GPIO_Pin_7;
		}
#endif
#endif
 ...
 *                   - GPIO_PIN_HNIB:Pin High 8 Bits selected
 *                   - GPIO_PIN_All:All pins selected
 * @param  GPIO_Mode[in]: specifies the operating Drive Level for the selected pins.
 ...
 * @retval None
 */
void GPIO_ModeConfig ( GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, GPIO_Mode_TypeDef GPIO_Mode )
{
		/* Check the parameters */
    assert_param ( IS_GPIO_ALL_PERIPH ( GPIOx ) );
    ...
    {
        /* Configure Pins to High-resistance output mode */
        GPIOx->PXCON |= ( uint32_t ) GPIO_Pin;
    }
    else if ( GPIO_Mode == GPIO_Mode_IN_PU )
    {
        /* Configure Pins to Pull-up input mode */
        GPIOx->PXCON &= ( uint32_t ) ( ~GPIO_Pin );
        GPIOx->PXPH |= ( uint32_t ) GPIO_Pin;
    }
    else if ( GPIO_Mode == GPIO_Mode_IN_HI )
    {
        /* Configure Pin to High-resistance intput mode */
        GPIOx->PXCON &= ( uint32_t ) ( ~GPIO_Pin );
        GPIOx->PXPH &= ( uint32_t ) ( ~GPIO_Pin );
