...
void INT_Init ( INT_InitTypeDef* INT_InitStruct )
{
    ...
    assert_param ( IS_INT_INTSEL ( INT_InitStruct->INT_INTSEL ) );
		...
		/* If the current initialization structure's channel settings include the INT channel on the UART TX pin of the programming port,
	     then cancel this channel configuration */
#if (UARTOptSwitch)
#if defined(SC32f10xx)
		if(((INT_InitStruct->INT_Channel & INT_Channel_3)  == INT_Channel_3) && (INT_InitStruct->INT_INTSEL == INT_INTSEL_PC))
		{
				INT_InitStruct->INT_Channel &= ~INT_Channel_3;
		}
#elif defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806)
		if(((INT_InitStruct->INT_Channel & INT_Channel_1)  == INT_Channel_1) && (INT_InitStruct->INT_INTSEL == INT_INTSEL_PB))
		{
				INT_InitStruct->INT_Channel &= ~INT_Channel_1;
		}
#elif defined(SC32f15xx) || defined(SC32R601)
		if(((INT_InitStruct->INT_Channel & INT_Channel_8)  == INT_Channel_8) && (INT_InitStruct->INT_INTSEL == INT_INTSEL_PC))
		{
				INT_InitStruct->INT_Channel &= ~INT_Channel_8;
		}
#elif defined(SC32R803)
		if(((INT_InitStruct->INT_Channel & INT_Channel_5)  == INT_Channel_5) && (INT_InitStruct->INT_INTSEL == INT_INTSEL_PD))
		{
				INT_InitStruct->INT_Channel &= ~INT_Channel_5;
		}
#elif defined(SC32L14xx)||defined(SC32R807)
		if(((INT_InitStruct->INT_Channel & INT_Channel_7)  == INT_Channel_7) && (INT_InitStruct->INT_INTSEL == INT_INTSEL_PE))
		{
				INT_InitStruct->INT_Channel &= ~INT_Channel_7;
		}
#endif
#endif
 ...
 *                   - INT_Channel_7:Int Select: Px7
 *                   - INT_Channel_8:Int Select: Px8
 *                   - INT_Channel_1:Int Select: Px9
 *                   - INT_Channel_9:Int Select: Px10
 *                   - INT_Channel_10:Int Select: Px11
 *                   - INT_Channel_11:Int Select: Px12
 *                   - INT_Channel_12:Int Select: Px13
 *                   - INT_Channel_13:Int Select: Px14
 *                   - INT_Channel_14:Int Select: Px15
 *                   - INT_Channel_15:Int Select: Px16
...
{
    /* Check the parameters */
    assert_param(IS_INT_CHANFLAG(INT_Channel));

    uint32_t mask = INT_Channel;
    uint8_t bit_pos = 0;
    while (mask != 0)
    {
        if (mask & 0x01)
