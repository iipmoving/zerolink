/**
 * appcooking.c —— AppCooking 模块实现 (v2.3)
 *
 * 依赖: include/appcooking_io.h
 * 层级: app
 *
 ...
 *   3. 业务逻辑函数
 */
#include "include/appcooking_io.h"  /* 包含 std_module.h + 三层结构定义 */
#include "appcooking.h"
#include <stddef.h>

/* ---- v2.3 三层数据结构 ---- */
/* PARAMS 实例 (纯数据) - TODO: 添加字段初始化 */
static MODULE_OUTPUT_PARAMS(AppCooking, AppPower) s_params_apppower;


/* LINK 实例 (status + *params) */
static MODULE_OUTPUT_LINK(AppCooking, AppPower) s_link_apppower;


/* OUTPUT 实例 (多LINK指针聚合) - Producer 专用 */
static MODULE_OUTPUT(AppCooking) s_out;

/* INPUT 实例 — Consumer 的输入由 Switcher 通过 InputCallback 赋值 */
static MODULE_INPUT(AppCooking) s_in;
...
void appcooking_Init(void) { Constructor(); }
