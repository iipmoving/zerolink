...
static void debug_show_key_event(uint8_t key, uint8_t evt)
{
    ...
    if (evt == KEY_STATE_RELEASE) {
        ...
        if (evt == KEY_STATE_PRESS || evt == KEY_STATE_LONG
            || evt == KEY_STATE_REPEAT) {
    ...
    if (evt != KEY_STATE_RELEASE) {
        if (evt == KEY_STATE_TAP) {
            s_display.seg_chars[6] = '1';
        } else if (evt == KEY_STATE_LONG) {
            s_display.seg_chars[6] = '2';
        } else if (evt == KEY_STATE_REPEAT) {
...
static void on_key_event(MsgId_t id, uint16_t param, void *data_ptr)
{
    ...
    if (s_led_test_active) {
        if (key == KEY_ONOFF && evt == KEY_STATE_LONG) {
            s_led_test_active = 0;
            debug_show_key_event(0, 0);
            return;
        }
        if (key == KEY_ADD && evt == KEY_STATE_PRESS) {
            s_led_test_index++;
        ...
        if (key == KEY_SUB && evt == KEY_STATE_PRESS) {
    ...
    /* 只处理 TAP 和 LONG */
    if (evt != KEY_STATE_TAP && evt != KEY_STATE_LONG) {
        return;
    }
    event_type = (evt == KEY_STATE_TAP) ? HMI_EVT_TAP : HMI_EVT_LONG;
...
static void post_buzzer(uint8_t valid)
{
    /* 通过消息总线解耦: APP → MSG_BUZZER_CTRL → DRV */
    Msg_Post(MSG_BUZZER_CTRL, valid ? 1u : 0u, NULL);
}
...
{
    switch (key) {
    case KEY_LEFT_P_SET_UP:   return 0;  /* 上左 → Z1 */
    case KEY_RIGHT_P_SET_UP:  return 1;  /* 上右 → Z2 */
    case KEY_LEFT_P_SET:      return 2;  /* 下左 → Z3 */
    case KEY_RIGHT_P_SET:     return 3;  /* 下右 → Z4 */
    default:                  return -1;
    }
}
...
{
    switch (idx) {
    case 0: return KEY_LEFT_P_SET_UP;   /* Z1 → 上左 */
    case 1: return KEY_RIGHT_P_SET_UP;  /* Z2 → 上右 */
    case 2: return KEY_LEFT_P_SET;      /* Z3 → 下左 */
    case 3: return KEY_RIGHT_P_SET;     /* Z4 → 下右 */
...
void engine_init(void)              { App_Hmi_Init(); }

void engine_post_key(uint8_t key, uint8_t evt)
{
    uint16_t param = ((uint16_t)evt << 8) | key;
    on_key_event(MSG_KEY_EVENT, param, NULL);
}

void engine_tick_100ms(void)
{
    on_timer_100ms(MSG_TIMER_100MS, 0, NULL);
}

void engine_tick_1s(void)
{
    on_timer_1s(MSG_TIMER_1S, 0, NULL);
}

/* ===== WASM 导出: 状态查询 ===== */

uint8_t engine_get_global_mode(void)   { return s_global.mode; }
uint8_t engine_is_child_lock(void)     { return s_global.child_lock; }
uint8_t engine_is_paused(void)         { return s_global.paused; }
int8_t  engine_get_hot_head(void)      { return s_hot_head; }
uint8_t engine_get_stack_depth(void)   { return s_stack_count; }
...
{
    if (idx < 4u && s_heads[idx].timer_active && s_heads[idx].timer_value > 0) {
...
uint8_t engine_get_seg_mode(void)      { return s_display.seg_mode; }

uint8_t engine_get_led_power(void)     { return s_display.leds_power; }
uint8_t engine_get_led_timer(void)     { return s_display.leds_timer; }
uint8_t engine_get_led_pause(void)     { return s_display.leds_pause; }
uint8_t engine_get_led_child_lock(void){ return s_display.leds_child_lock; }
