...
static const KeyMask_t s_state_key_masks[STATE_MAX] = {
    ...
    /* S_VERSION = 1: 仅电源键或START_PAUSE快速开机（单按键设计） */
    [S_VERSION]     = { .short_mask = MASK_KEY_POWER | MASK_KEY_START_PAUSE, .long_mask = 0 },

    /* S_SHUTDOWN = 2: 电源键或START_PAUSE短按开机（单按键设计） */
    [S_SHUTDOWN]    = { .short_mask = MASK_KEY_POWER | MASK_KEY_START_PAUSE, .long_mask = 0 },

    /* S_DEMO = 3: 电源键或START_PAUSE退出演示（单按键设计） */
    [S_DEMO]        = { .short_mask = MASK_KEY_POWER | MASK_KEY_START_PAUSE, .long_mask = 0 },

    /* S_STANDBY = 4: M1-M10选择功能 + ADD_TIME调整时间 + START_PAUSE长按关机（单按键设计） */
    [S_STANDBY]     = {
        .short_mask = MASK_M1_M10 | MASK_KEY_ADD_TIME,
        .long_mask  = MASK_KEY_POWER | MASK_KEY_START_PAUSE  /* 长按关机 */
    },
 ...
 * ========================================================================
 *
 * 转移表 s_state_trans_table[9][33] 占用 9×33×2 = 594 字节。
 * 8051 内部 IDATA 仅 256 字节，无法容纳此表。
 *
 * 优化方案：
 *   a) 使用 __xdata（外部 RAM，适合有 XDATA 的 8051 型号）— 默认方案
 *   b) 定义 EMC_OPT_TRANS_TABLE_IN_CODE 将表放入 CODE（Flash）
 *      需编译器支持 C99 指定初始化器（KEIL C51 v9.60+ / SDCC 4.0+）
 *      启用后零初始化条目 = "保持状态+无动作"（安全默认值）
 *
 * 本实现采用方案(a)：__xdata 外部 RAM。未定义 __C51__ 时使用默认内存。
 * ======================================================================== */

#ifdef __C51__
  #define EMC_XDATA  __xdata
#else
  #define EMC_XDATA
#endif
...
static EMC_XDATA StateTransition_t s_state_trans_table[STATE_MAX][EVENT_MAX];  /* 转移表 */
static uint8_t s_any_key_processed;                /* 本周期是否有按键被处理 */

/* ========================================================================
 * 调试变量 - 供dll_wrapper访问
 * ======================================================================== */
uint8_t s_debug_last_key_code = 0;
uint8_t s_debug_last_event = 0;
uint8_t s_debug_mask_valid = 0;
uint8_t s_debug_lock_valid = 0;
uint8_t s_debug_state_before = 0;
uint8_t s_debug_state_after = 0;
...
void key_disp_cycle(void) {
    ...
    while (s_key_buffer.head != s_key_buffer.tail) {
        ...
        /* 保存调试信息 */
        s_debug_last_key_code = p_evt->key_code;
        s_debug_last_event = p_evt->event;
        s_debug_state_before = s_emc.current_state;

        /* End阶段特殊处理：任意有效按键 → 标记退出 */
        ...
        /* 2.1 掩码检查 */
        s_debug_mask_valid = is_key_valid(p_evt->key_code, p_evt->event);
        
        /* 2.2 锁定检查 */
        s_debug_lock_valid = 0;
        if (s_debug_mask_valid) {
            s_debug_lock_valid = check_lock_conditions(p_evt);
        }
        
        /* 2.3 状态机处理 */
        if (s_debug_mask_valid && s_debug_lock_valid) {
            process_key_event(p_evt);
            s_any_key_processed = 1;
        }

        s_debug_state_after = s_emc.current_state;
 ...
 */
static uint8_t map_key_to_transition_event(uint8_t key_code, uint8_t key_event) {
    uint8_t is_short = (key_event == KEY_EVENT_SHORT || key_event == KEY_EVENT_PRESS);
    uint8_t is_long  = (key_event == KEY_EVENT_LONG  || key_event == KEY_EVENT_REPEAT);

    if (key_code == KEY_POWER) {
        if (is_short) { return EV_KEY_POWER_SHORT; }
        if (is_long)  { return EV_KEY_POWER_LONG;  }
    }
    /* KEY_START_PAUSE 兼作电源键：在电源相关状态下映射为电源事件 */
    if (key_code == KEY_START_PAUSE) {
        /* S_SHUTDOWN / S_DEMO: 短按=开机(等同POWER短按) */
        if (s_emc.current_state == S_SHUTDOWN || s_emc.current_state == S_DEMO) {
            if (is_short) { return EV_KEY_POWER_SHORT; }
        }
        /* S_VERSION: 短按=快速开机(等同POWER短按) */
        if (s_emc.current_state == S_VERSION) {
            if (is_short) { return EV_KEY_POWER_SHORT; }
        }
        /* S_STANDBY: 长按=关机(等同POWER长按) */
        if (s_emc.current_state == S_STANDBY) {
            if (is_long)  { return EV_KEY_POWER_LONG; }
        }
    }

    if (key_code == KEY_START_PAUSE) {
        if (is_short) { return EV_KEY_START_PAUSE_SHORT; }
        if (is_long)  { return EV_KEY_START_PAUSE_LONG;  }
    }
...
static void execute_action(uint8_t action, const KeyEvent_t *p_evt) {
    ...
    case ACTION_ADJUST_WATER:
        ...
        s_emc.flags.show_time_disp = 0;   /* 切换到水量显示 */
        /* 刷新显示 */
        update_display();
    ...
    case ACTION_ADJUST_TIME:
        ...
        s_emc.flags.show_time_disp = 1;   /* 切换到时间显示 */
        update_display();
        request_buzzer(BUZZ_CLICK);
...
static void update_display(void) {
    ...
    case S_FUNC_SELECT:
        if (s_emc.flags.show_time_disp) {
            /* 显示当前设定时间 X:XX */
            sec = s_emc.params.current_time_s;
            min = (uint8_t)(sec / 60);
            ss  = (uint8_t)(sec % 60);
...
static void update_led_state(void) {
    ...
    case S_FUNC_SELECT:
        ...
        /* 根据show_time_disp标志位设置显示指示灯 */
        if (s_emc.flags.show_time_disp) {
            s_emc.led.time_disp_led  = 1;  /* 显示时间时，时间指示灯亮 */
            s_emc.led.water_disp_led = 0;
        } else {
            s_emc.led.water_disp_led = 1;  /* 显示水量时，水量指示灯亮 */
            s_emc.led.time_disp_led  = 0;
        }
        break;
