...
async function startDemo(logicType = 'js') {
    try {
        ...
        } else if (logicType === 'json') {
            await startDemoWithJSON();
        } else {
            await startDemoWithJS();
        }
    } catch (error) {
 ...
 * 使用JSON逻辑层启动演示
 */
async function startDemoWithJSON() {
    try {
        log('正在初始化JSON逻辑层适配器...', 'info');
        
        // 加载JSON逻辑层配置
        const logicResponse = await fetch('configs/emc_controller_v2.0_standard.json');
        if (!logicResponse.ok) {
            throw new Error(`HTTP error! status: ${logicResponse.status}`);
        }
        const jsonConfig = await logicResponse.json();
        console.log('[startDemoWithJSON] JSON逻辑层配置加载成功:', jsonConfig.meta);
        
        // 加载界面配置（fourSave.json）
        let panelConfig = null;
        if (AppState.config && AppState.config.controls) {
            // 使用当前已加载的界面配置
            panelConfig = AppState.config;
            console.log('[startDemoWithJSON] 使用当前界面配置');
        } else {
            // 尝试加载默认的fourSave.json
            try {
                const panelResponse = await fetch('prj/lib1/fourSave.json');
                if (panelResponse.ok) {
                    panelConfig = await panelResponse.json();
                    console.log('[startDemoWithJSON] 默认界面配置加载成功');
                }
            } catch (e) {
                console.warn('[startDemoWithJSON] 无法加载界面配置，将使用默认映射', e);
            }
        }
        
        // ✅ 创建JSON逻辑层适配器（传入界面配置）
        AppState.logicLayer = new JsonLogicAdapter(jsonConfig, panelConfig);
        ...
        AppState.logicLayer.registerCallbacks(
            ...
            (cmd) => {
                // 蜂鸣器回调
                console.log(`[JsonLogicAdapter] 🔊 BUZZER: cmd=${cmd}`);
            },
            (type, valPtr) => {
                // 状态变化回调
                if (type === 1 && valPtr) {
                    const newState = typeof valPtr === 'number' ? valPtr : valPtr;
                    AppState.dllState.state = newState;
                    console.log(`[JsonLogicAdapter] 🔄 状态变化: ${newState}`);
                    updateDebugPanel();
                }
            },
            null // 硬件输入回调（demo模式不使用）
        ...
        log('演示模式已启动（JSON逻辑层）', 'success');
        AppState.mode = 'demo';
        
    } catch (error) {
        log(`启动演示模式失败: ${error.message}`, 'error');
        console.error('startDemoWithJSON错误详情:', error);
        throw error;
    }
}
...
function setupKeyConfigs() {
    ...
    // ✅ 硬编码设置：将START_PAUSE键设为长按功能按键
    // （本来应该由用户在控件编辑菜单中设置，现在程序自动设置）
    if (AppState.controls) {
        AppState.controls.forEach(control => {
            if (control.config && control.config.key_code === 13) { // KEY_START_PAUSE = 13
                control.config.long_press = true;
                log(`✅ 已硬编码设置 ${control.name} (KeyCode=13) 为长按功能按键`, 'info');
            }
        });
    }
...
function setupCanvasEvents() {
    ...
    canvas.removeEventListener('mousedown', AppState.canvasMouseDownHandler);
    canvas.removeEventListener('mouseup', AppState.canvasMouseUpHandler);
    console.log('[setupCanvasEvents] 已移除所有旧的事件监听器');
    ...
    AppState.canvasMouseDownHandler = (event) => {
        console.log('[DEBUG] mousedown事件触发');
        const rect = canvas.getBoundingClientRect();
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top;
        
        console.log(`[DEBUG] 点击坐标: (${x.toFixed(0)}, ${y.toFixed(0)})`);
        
        // 查找被点击的控件
        const control = findControlByPosition(x, y);
        if (!control || control.type !== 'button') {
            console.log('[DEBUG] 未找到按键控件或不是button类型');
            return;
        }
        
        console.log(`[DEBUG] 找到按键控件: ${control.name}, type=${control.type}`);
        
        if (!control.config || !control.config.key_code) {
            console.log('[DEBUG] 控件没有config或key_code');
            return;
        }
        ...
        console.log(`[DEBUG] keyCode=${keyCode}, isLongPressKey=${isLongPressKey}`);
        console.log(`[DEBUG] AppState.logicLayer存在: ${!!AppState.logicLayer}`);
        console.log(`[DEBUG] AppState.logicLayer类型: ${AppState.logicLayer?.constructor?.name}`);
        ...
        if (AppState.logicLayer) {
            console.log(`[DEBUG] 准备调用logicLayer.pressKey(${keyCode}, ...)`);
            ...
            if (isLongPressKey) {
                ...
                // 普通按键：按下时立即发送PRESS事件
                console.log(`[DEBUG] 调用pressKey: keyCode=${keyCode}, event=PRESS`);
                AppState.logicLayer.pressKey(keyCode, KeyEvent.KEY_EVENT_PRESS);
                console.log(`[DEBUG] pressKey调用完成`);
...
async function startDemoWithWasm() {
    try {
        ...
        // ✅ 关键修复：将wasmAdapter赋值给logicLayer，确保统一接口
        AppState.logicLayer = AppState.wasmAdapter;
        console.log('[startDemoWithWasm] ✅ AppState.logicLayer已设置为WASM适配器');
        ...
        console.log('[DEBUG] setupWasmCanvasClick completed');
 ...
 * 设置WASM Canvas点击事件
 */
function setupWasmCanvasClick() {
    const canvas = AppState.canvas;
    if (!canvas || !AppState.controlBinder) return;
    ...
    canvas.removeEventListener('mousedown', AppState.wasmMouseDownHandler);
    canvas.removeEventListener('mouseup', AppState.wasmMouseUpHandler);
    console.log('[DEBUG] 已移除所有旧的事件监听器');
    
    // ✅ WASM模式下的按键按下处理
    AppState.wasmMouseDownHandler = (event) => {
        console.log(`[DEBUG] mousedown at (${event.clientX}, ${event.clientY})`);
        
        const rect = canvas.getBoundingClientRect();
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top;
        
        console.log(`[DEBUG] Canvas相对坐标: (${x}, ${y})`);
        
        // 查找被点击的控件
        const control = findControlByPosition(x, y);
        if (!control) {
            console.log(`[DEBUG] 没有找到控件`);
            return;
        }
        
        console.log(`[DEBUG] 找到控件: ${control.name}`);
        
        if (!control.config || !control.config.key_code) {
            console.log(`[DEBUG] 控件没有配置或键码`);
            return;
        }
        ...
        const pressTime = Date.now();
        AppState.pressedKeys[control.name] = pressTime;
        log(`🔍 WASM ${control.name} 按下时间=${pressTime}, long_press=${isLongPressKey}`, 'debug');
        
        if (AppState.wasmAdapter) {
            if (isLongPressKey) {
                // ✅ 长按功能按键：按下时不发送任何事件，等待松开时判断
                log(`🔘 WASM [长按模式] 按键按下: ${control.name} (KeyCode=${keyCode})`, 'info');
            } else {
                // ✅ 普通按键：按下时立即发送PRESS事件
                AppState.wasmAdapter.pressKey(keyCode, KeyEvent.KEY_EVENT_PRESS);
                log(`🔘 WASM按键按下: ${control.name} (KeyCode=${keyCode})`, 'info');
            }
        }
    };
    
    // ✅ WASM模式下的按键松开处理
    AppState.wasmMouseUpHandler = (event) => {
        console.log(`[DEBUG] mouseup at (${event.clientX}, ${event.clientY})`);
        ...
        const control = findControlByPosition(x, y);
        if (!control) {
            console.log(`[DEBUG] mouseup没有找到控件`);
            return;
        }
        
        console.log(`[DEBUG] mouseup找到控件: ${control.name}`);
        
        if (!control.config || !control.config.key_code) {
            console.log(`[DEBUG] mouseup控件没有配置或键码`);
            return;
        }
        ...
        const pressTime = AppState.pressedKeys[control.name];
        if (!pressTime) {
            log(`⚠️ WASM ${control.name} 没有找到按下时间记录`, 'warn');
            return;
        }
        ...
        log(`🔍 WASM ${control.name} 时长=${elapsed}ms, long_press=${isLongPressKey}`, 'debug');
        
        // ✅ 更新界面上的按键信息
        if (window.updateKeyInfo) {
            window.updateKeyInfo(control.name, keyCode, null, elapsed, isLongPressKey);
        }
        ...
        if (AppState.wasmAdapter) {
            if (isLongPressKey) {
                ...
                if (elapsed < 1500) {
                    // 短按：发送SHORT事件
                    log(`📤 WASM [长按模式] ${control.name} 发送SHORT (keyCode=${keyCode}, event=2)`, 'info');
                    AppState.wasmAdapter.pressKey(keyCode, KeyEvent.KEY_EVENT_SHORT);
                    
                    // 更新界面显示事件类型
                    if (window.updateKeyInfo) {
                        window.updateKeyInfo(control.name, keyCode, 2, elapsed, isLongPressKey);
                    }
                } else {
                    // 长按：发送LONG事件
                    log(`📤 WASM [长按模式] ${control.name} 发送LONG (keyCode=${keyCode}, event=3)`, 'info');
                    AppState.wasmAdapter.pressKey(keyCode, KeyEvent.KEY_EVENT_LONG);
                    
                    // 更新界面显示事件类型
                    if (window.updateKeyInfo) {
                        window.updateKeyInfo(control.name, keyCode, 3, elapsed, isLongPressKey);
                    }
                }
            } else {
                // ✅ 普通按键：根据时长判断SHORT或LONG
                if (elapsed < 1500) {
                    log(`📤 WASM ${control.name} 发送SHORT (keyCode=${keyCode}, event=2)`, 'info');
                    AppState.wasmAdapter.pressKey(keyCode, KeyEvent.KEY_EVENT_SHORT);
                } else {
                    log(`📤 WASM ${control.name} 发送LONG (keyCode=${keyCode}, event=3)`, 'info');
                    AppState.wasmAdapter.pressKey(keyCode, KeyEvent.KEY_EVENT_LONG);
                }
            }
            
            // ✅ 运行多个周期让WASM处理按键（确保事件被处理）
            for (let i = 0; i < 5; i++) {
                AppState.wasmAdapter.runCycle();
            }
            log(`   🔄 已运行5个周期处理按键事件`, 'debug');
        }
    };
    
    // ✅ 保留原有的click事件作为后备
    AppState.wasmClickHandler = (event) => {
        const rect = canvas.getBoundingClientRect();
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top;
        
        if (AppState.controlBinder.handleCanvasClick(x, y)) {
            log(`👆 点击坐标: (${x.toFixed(0)}, ${y.toFixed(0)})`, 'info');
            
            // ✅ 单向数据流：不需要手动更新显示
            // WASM内部的disp_output_callback会自动推送新状态
            setTimeout(() => {
                const state = AppState.wasmAdapter.getState();
                const stateName = AppState.wasmAdapter.STATE_NAMES[state] || `UNKNOWN(${state})`;
                log(`   当前状态: ${stateName}`, 'info');
            }, 50);
        }
    };
    
    // ✅ 优先使用mousedown/mouseup，支持长按
    canvas.addEventListener('mousedown', AppState.wasmMouseDownHandler);
    canvas.addEventListener('mouseup', AppState.wasmMouseUpHandler);
    // ❌ 移除click事件，避免与mousedown/mouseup冲突
    // canvas.addEventListener('click', AppState.wasmClickHandler);
    
    log('✅ WASM Canvas事件已绑定（支持长按）', 'info');
}
...
function stopWasmDemo() {
    try {
        ...
        // 移除Canvas点击事件
        if (AppState.canvas && AppState.wasmClickHandler) {
            AppState.canvas.removeEventListener('click', AppState.wasmClickHandler);
            AppState.wasmClickHandler = null;
        }
