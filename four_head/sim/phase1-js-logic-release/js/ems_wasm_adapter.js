...
class EmsWasmAdapter extends LogicLayerAdapter {
    constructor(wasmPath = 'wasm/emc_test.js') {
        ...
        // LED状态推断缓存
        this._lastStateForLedInference = -1;
     ...
     */
    getLedState() {
        // ✅ 尝试从WASM读取（如果导出了emc_get_led_state_ptr）
    ...
    _inferLedState(state) {
        ...
        // 根据状态机逻辑推断LED
        switch (state) {
            case this.STATES.S_STANDBY:  // 待机状态
                led.power = true;        // 电源灯亮
                led.func_leds = 0x3FF;   // M1-M10全亮（与C代码一致）
                break;
                
            case this.STATES.S_FUNC_SELECT:  // 功能选择
                led.power = true;
                // M1-M10的功能灯由用户选择决定，这里无法准确判断
                // 暂时假设M1被选中（func_leds bit 0 = 1）
                led.func_leds = 0x01;  // M1灯亮
                break;
                
            case this.STATES.S_RUNNING:  // 运行中
                led.power = true;
                // 根据具体功能点亮对应的LED
                break;
