...
The methodology-seed lives at `../methodology-seed/` (relative to this project).
Adjust the path based on where the new project is created relative to the cluster root.

| Source | Path |
|--------|------|
| Templates | `../methodology-seed/templates/` |
| Tools | `../methodology-seed/tools/` |
| Method docs | `../methodology-seed/` |
...
PROJECT_DIR/
  ...
  core/         # Core/Message infrastructure
...
From `../methodology-seed/templates/`, copy and fill these files:
...
From `../methodology-seed/tools/`, copy these files to `PROJECT_DIR/tools/`:
...
Write-Host "[1/3] check_deps.py ... " -NoNewline
...
if (Test-Path "cfg/interface_map.h") {
    Write-Host "[2/3] check_weak_pairs.py ... " -NoNewline
    ...
    Write-Host "[2/3] check_weak_pairs.py ... SKIP (no interface_map.h)" -ForegroundColor Yellow
}

# 3. Struct consistency
if (Test-Path "cfg/structs.json") {
    Write-Host "[3/3] check_structs.py ... " -NoNewline
    ...
    Write-Host "[3/3] check_structs.py ... SKIP (no structs.json)" -ForegroundColor Yellow
}
...
if [ -f "cfg/interface_map.h" ]; then
    echo -n "[2/3] check_weak_pairs.py ... "
    ...
    echo -e "[2/3] check_weak_pairs.py ... \033[33mSKIP (no interface_map.h)\033[0m"
fi

# 3. Struct consistency
if [ -f "cfg/structs.json" ]; then
    echo -n "[3/3] check_structs.py ... "
    ...
    echo -e "[3/3] check_structs.py ... \033[33mSKIP (no structs.json)\033[0m"
fi
...
Next steps:
  1. Fill .claude/specs/tech-stack.md with actual hardware details
  2. Fill .claude/specs/ubiquitous-language.md with domain terms
