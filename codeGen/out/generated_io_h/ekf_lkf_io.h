 ...
 * @layer   base_class
 *
 * EKF_LKF Data Switcher IO interface (v2.3 LINK+PARAMS)
 *
 * 输入源:
 *   ElecParams → EKF_LKF  ()
 *
...
typedef struct {
    ...
    uint8_t  res[1];
    MODULE_OUTPUT_PARAMS(EKF_LKF, AppPower) *params;
...
typedef struct {
    ...
    uint8_t  count;            /* 当前周期索引 */
    uint8_t  res[1];
    MODULE_INPUT_PARAMS(ElecParams, EKF_LKF) params[0];
