 ...
 * @layer   base_class
 *
 * ElecParams Data Switcher IO interface (v2.3 LINK+PARAMS)
 *
 * 输入源:
...
typedef struct {
    ...
    uint8_t  res[1];
    MODULE_OUTPUT_PARAMS(ElecParams, AppPower) *params;
} MODULE_OUTPUT_LINK(ElecParams, AppPower);

/* ------------------------------------------------------------------
 * ElecParams → EKF_LKF  输出参数  ()
 * ------------------------------------------------------------------ */
typedef struct {
...
typedef struct {
    ...
    uint8_t  count;            /* 当前周期索引 */
    uint8_t  res[1];
    MODULE_OUTPUT_PARAMS(ElecParams, EKF_LKF) *params;
} MODULE_OUTPUT_LINK(ElecParams, EKF_LKF);

/* ------------------------------------------------------------------
 * ElecParams → Telemetry  输出参数  ()
 * ------------------------------------------------------------------ */
typedef struct {
...
typedef struct {
    ...
    uint8_t  count;            /* 当前周期索引 */
    uint8_t  res[1];
    MODULE_OUTPUT_PARAMS(ElecParams, Telemetry) params[0];
