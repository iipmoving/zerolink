 ...
 * @layer   app
 *
 * AppPower Data Switcher IO interface (v2.3 LINK+PARAMS)
 *
 * 输入源:
 *   AppAdc → AppPower  ()
 *   ElecParams → AppPower  ()
...
typedef struct {
    ...
    uint8_t  count;            /* 当前周期索引 */
    uint8_t  res[1];
    MODULE_INPUT_PARAMS(EKF_LKF, AppPower) params[0];
