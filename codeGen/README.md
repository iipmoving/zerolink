...
**核心原则：生成器只加不减，不动原文件内容。** 原有代码（包括未接入 AI 引导段的旧程序）在生成过程中绝对保留。AI 后续手动对接接口、替换入口。

所有入口（全部生成 / 代码预览更新）执行同一套逻辑：

### 规则 1：新文件（不存在）→ 完整生成

目标文件不存在时，生成完整的文件内容，包含 `#include`、`#pragma pack`、`MODULE_IO_H` 等全部结构。

示例：`AppAdc_io.h` 首次生成 → 输出完整的头文件。

### 规则 2：已存在 + 有 `[AI GENERATED]` 引导段（稳态）→ 只替换 AI 段

目标文件已有 `// ===== [AI GENERATED] ... =====` 标记，且标记前包含标准头部（`#ifndef`/`#include` 等）：
- 保留标记前的头部区域（含用户添加的常量、宏定义）
- 保留标记后的尾部区域（`MODULE_IO_H`、`#endif` 等）
- **只替换** `[AI GENERATED]` ↔ `[END AI GENERATED]` 之间的内容
...
### 规则 3：已存在 + 有 `[AI GENERATED]` 引导段（迁移）→ 旧 AI 块替换为新完整内容

目标文件有 `[AI GENERATED]` 标记，但标记前**没有**标准头部（旧版本生成的旧格式，整个文件在 AI 块内）：
- 保留用户添加在旧 AI 标记前的内容（如常量定义）
- **整个旧 AI 块替换为**新生成的完整内容（头部 + AI 段 + 尾部）
- 经过一次更新后，文件进入稳态，后续更新走规则 2

```
┌─ // 用户常量 ← 保留（旧 AI 块前的内容）
├─ [AI GENERATED]  旧格式(全文件)  ┐
├─   #ifndef ...                    │ 替换为
├─   ...全部内容...                  │ 新生成完整内容
├─   #endif                         │ (头部+AI段+尾部)
├─ [END AI GENERATED]               ┘
...
### 规则 4：已存在 + 无 `[AI GENERATED]` 引导段（原有程序）→ 插入到最前面

目标文件存在但没有 `[AI GENERATED]` / `[END AI GENERATED]` 标记（如手写的原始文件）：
- **整个生成内容插入到原文件最前面**
- 原文件内容保留不动
- 这确保原有代码不受影响，同时注入 AI 可管理的引导段

```
┌─ [AI GENERATED]  完整生成内容    ← 插入到最前面
├─   #ifndef ...
├─   ...
├─ [END AI GENERATED]
├─ // 原文件原有内容              ← 原样保留
