...
# /modify-module — v1.x → v2.3 转换向导

将现有模块改造为符合 v2.3 PULL 架构（`std_module.h` 模式）。
...
typedef struct {
    MODULE_OUTPUT_LINK(Key, Hmi) *Hmi_params;
...
typedef struct {
    MODULE_INPUT_LINK(Key, Hmi)   *DrvKey_params;
    MODULE_INPUT_LINK(Power, Hmi) *AppPower_params;
} MODULE_INPUT(AppHmi);
...
typedef struct {
    MODULE_OUTPUT_LINK(Hmi, Display) *Display_params;
...
INPUT_CALLBACK(Calculator, PowerBase)
{
    ...
    pwr_in->Calculator_params = (void*)&ep_out->ElecParams_params;
}
```
