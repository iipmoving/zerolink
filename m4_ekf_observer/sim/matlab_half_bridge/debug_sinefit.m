% debug_sinefit — 最小二乘正弦拟合提取基波, 铁锅 vs 钢锅
function debug_sinefit()
...
for p = 1:2
    ...
    for fi = 1:length(frames_idx)
        ...
        % 最小二乘正弦拟合: I(t) = A*cos(ωt) + B*sin(ωt) + C
        % 设计矩阵 X = [cos(ωt), sin(ωt), 1]
        X = [cos(omega * t_sec), sin(omega * t_sec), ones(n_trim, 1)];
        coeff = X \ I_trim;  % 最小二乘解
        A = coeff(1); B = coeff(2); C = coeff(3);
        I_f1_peak = sqrt(A^2 + B^2);
        I_rms_f1 = I_f1_peak / sqrt(2);

        % 拟合残差 → THD
        I_fit = X * coeff;
        I_res = I_trim - I_fit;
        THD = rms(I_res) / I_rms_f1 * 100;
        ...
        if ph.usable && I_rms_tot > 0.1
            Z_tot = V1 / I_rms_tot;
            L_tot = (1/(omega*C_FARAD) + Z_tot*sind(ph.phi_deg)) / omega * 1e6;
        else, L_tot = NaN; end

        if ph.usable && I_rms_f1 > 0.1
            Z_f1 = V1 / I_rms_f1;
            L_f1 = (1/(omega*C_FARAD) + Z_f1*sind(ph.phi_deg)) / omega * 1e6;
        else, L_f1 = NaN; end
