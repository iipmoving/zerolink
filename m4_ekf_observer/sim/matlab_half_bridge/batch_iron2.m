...
for fidx = 1:length(csvs)
    ...
    rs = {};
    for f = 1:length(fd)
        r = calc_one_frame_iron(fd{f}, fN{f});
        rs{end+1} = r;
    end
    all_r{fidx} = rs;
end
...
%% 总体
fprintf('\n========== 铁锅第二轮总体 ==========\n');
all_L=[]; all_Q=[]; all_P=[]; all_phi=[]; all_fsw=[];
for fidx = 1:length(csvs)
    for fi = 1:length(all_r{fidx})
        r = all_r{fidx}{fi};
        if ~isnan(r.phi_deg) && r.I_peak_A > 2
...
%% 按频段分组
bands = {[20 25], [25 27], [27 29], [29 32], [32 35], [35 40]};
fprintf('\n========== 分频段 ==========\n');
for b = 1:length(bands)
    lo = bands{b}(1); hi = bands{b}(2);
    bl=[]; bq=[]; bp=[]; bphi=[];
    for i = 1:length(all_L)
        if all_fsw(i) >= lo && all_fsw(i) < hi
            bl=[bl all_L(i)]; bq=[bq all_Q(i)]; bp=[bp all_P(i)]; bphi=[bphi all_phi(i)];
        end
    end
    if ~isempty(bl)
        fprintf('[%d-%dkHz] N=%d  L=%.1f+-%.1f uH  Q=%.2f+-%.2f  phi=%.1f+-%.1f  P=%.0f+-%.0f\n', ...
            lo, hi, length(bl), mean(bl), std(bl), mean(bq), std(bq), mean(bphi), std(bphi), mean(bp), std(bp));
    end
end

end

%% ---- 以下为 calc_* (同 run_per_frame 阻抗法) ----
...
function r = calc_one_frame_iron(frame, N)
