...
for f_idx = 1:length(CSV_FILES)
    ...
    for c = 1:length(cycles)
        ...
        % ---- 3e. I_peak: 窗口内峰值 + 趋势噪声滤除 ----
        % 窗口: [v_up, co_idx] = 第一半周期
        % v_up(谷值/过零点) = 上管开通后第一个过零点, 是真实导通的起点
        % CMP_UON→v_up 之间的电流是上一周期关断的残余振荡/噪音
        % co_idx(≈CMP_UOFF) = 上管关断, 之后可能有开关噪音
        win_start = min(v_up, n);
        win_end   = min(co_idx - 1, n);
        
        if win_end > win_start && v_up > start_i
            I_win = cur(win_start:win_end);
            [I_win_max, idx_win] = max(I_win);
            pk_abs_idx = win_start + idx_win - 1;
            
            % 趋势噪声滤除
            if pk_abs_idx >= 3 && pk_abs_idx <= n
                d2 = cur(pk_abs_idx-1) - cur(pk_abs_idx-2);
                d1 = cur(pk_abs_idx)   - cur(pk_abs_idx-1);
                if abs(d2) > 1 && abs(d1) > abs(d2) * 1.5
                    d3_max = max(cur(pk_abs_idx-3:pk_abs_idx-1));
                    d3_min = min(cur(pk_abs_idx-3:pk_abs_idx-1));
                    pred_d1 = d2 * 0.5;  % 趋势: 每一步减半
                    I_adc_max = round(cur(pk_abs_idx-1) + pred_d1);
                else
                    I_adc_max = I_win_max;
                end
            else
            ...
            I_adc_max = max(cur(cur < 60000));  % fallback
        end
    ...
    % I_peak: 用最大值作为参考 (高电流时 B-H 饱和, L 稳定)
    % 低 I_peak 周期向高 I_peak 参考修正
    [I_peak_max, idx_max] = max(all_I_peak);
    Vdc_ref = all_Vdc(idx_max);
    I_peak_ref = I_peak_max;
    V_C_peak_ref = I_peak_ref / (omega_sw * C_F);
    L_ref = (Vdc_ref/2 + V_C_peak_ref) / (I_peak_ref * omega_sw) * 1e6;

    % 逐周期修正: L_corr = L_ref + (L_raw - L_ref) × (I / I_max)^2
    % 丢弃 I_peak < I_max×0.1 的周期
    L_corr_all = [];
    for i = 1:length(all_cycles)
        ratio = all_I_peak(i) / I_peak_max;
        if ratio < 0.1
            continue;
        end
        weight = ratio^2;
        L_raw_i = all_cycles{i}.golden.L_uH;
        L_corr_i = L_ref + (L_raw_i - L_ref) * weight;
        L_corr_all(end+1) = L_corr_i;
    end
    if isempty(L_corr_all)
        L_corr_all = L_ref;
    end
    L_uH = median(L_corr_all);
