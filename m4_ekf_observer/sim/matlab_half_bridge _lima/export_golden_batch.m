% export_golden_batch.m — 批量 golden 导出 (ekf_tuner 全部 CSV)
% 用法: matlab -batch "run('export_golden_batch.m')"
% 输出: golden_batch.json

clear; clc;

CSV_DIR = 'D:\OBSIDIAN\MOVING IH\低耦合程序架构\m4_ekf_observer\tools\ekf_tuner\captures';
OUTPUT = fullfile(fileparts(mfilename('fullpath')), 'golden_batch.json');

% 找到所有 capture_*.csv (排除 *_result.csv)
listing = dir(fullfile(CSV_DIR, 'capture_*.csv'));
CSV_FILES = {};
for i = 1:length(listing)
    name = listing(i).name;
    if ~contains(name, '_result')
        CSV_FILES{end+1} = name;
    end
end

fprintf('[export_golden_batch] Found %d CSV files in %s\n', length(CSV_FILES), CSV_DIR);
...
for f_idx = 1:length(CSV_FILES)
    ...
    if ~exist(csv_path, 'file')
        fprintf('[export_golden_batch] SKIP: %s not found\n', csv_path);
        continue;
    end

    fprintf('[%d/%d] %s ...\n', f_idx, length(CSV_FILES), CSV_FILES{f_idx});
    ...
    while ~feof(fid)
        ...
        % data row: t_us, I_adc, V_adc, Vdc_adc, CNT, CU, CO, CL, LO (+ extra cols)
        all_t(end+1)   = t_val;
    ...
    for c = 1:length(cycles)
        ...
        % I_peak / I_rms: 窗口限 [v_up, co_idx-1], 排除关断前噪音
        win_start = min(v_up, n);
        win_end   = min(co_idx - 1, n);
        if win_end > win_start && v_up > start_i
            I_win = cur(win_start:win_end);
            I_adc_max = max(I_win);
        else
        ...
        % f_res: v_up->v_dn 谐振半周期
        f_res_kHz_i = f_sw_kHz;
    ...
    if isempty(all_cycles)
        fprintf('  WARNING: no valid cycles\n');
        continue;
    end

    % 收集每周期参数
    nC = length(all_cycles);
    all_P_W=zeros(1,nC); all_I_rms=zeros(1,nC); all_I_peak=zeros(1,nC);
    all_Vdc=zeros(1,nC); all_D_U=zeros(1,nC); all_phi=zeros(1,nC);
    all_f_sw=zeros(1,nC); all_f_res=zeros(1,nC); all_DT1=zeros(1,nC); all_DT2=zeros(1,nC);
    for i = 1:nC
        g = all_cycles{i}.golden;
        all_P_W(i)=g.P_W; all_I_rms(i)=g.I_rms; all_I_peak(i)=g.I_peak;
        all_Vdc(i)=g.Vdc_mean; all_D_U(i)=g.D_U_pct; all_phi(i)=g.phi_deg;
        all_f_sw(i)=g.f_sw_kHz; all_f_res(i)=g.f_res_kHz;
        all_DT1(i)=g.DT1_us; all_DT2(i)=g.DT2_us;
    end

    P_W_avg = mean(all_P_W); I_rms_avg = mean(all_I_rms);
    Vdc_avg = mean(all_Vdc); phi_avg = mean(all_phi);
    f_sw_avg = mean(all_f_sw);

    % KVL L 公式链
    C_uF = 0.9; C_F = C_uF * 1e-6;
    I_peak_avg = mean(all_I_peak);
    omega_sw = 2 * pi * f_sw_avg * 1e3;
    V_C_peak = I_peak_avg / (omega_sw * C_F);
    L_uH = (Vdc_avg/2 + V_C_peak) / (I_peak_avg * omega_sw) * 1e6;
    if L_uH < 0, L_uH = 0; end

    if L_uH > 0.001
        f_res_kHz = 1 / (2*pi*sqrt(L_uH*1e-6*C_F)) / 1e3;
    else
        f_res_kHz = f_sw_avg;
    end
    omega_res = 2 * pi * f_res_kHz * 1e3;
    tan_phi = tand(phi_avg);
    ratio = f_sw_avg / f_res_kHz;
    ...
    summary.cos_phi    = cosd(phi_avg);
    summary.D_U_pct    = mean(all_D_U);
    ...
    fprintf('  %d cycles, P=%.1fW, L=%.1fuH, f_res=%.2fkHz, Q=%.2f\n', ...
        nC, P_W_avg, L_uH, f_res_kHz, Q_factor);
end

% 导出 JSON
fprintf('[export_golden_batch] Writing %s ...\n', OUTPUT);
jsonStr = jsonencode(golden, 'PrettyPrint', true);
fid = fopen(OUTPUT, 'w');
fprintf(fid, '%s', jsonStr);
fclose(fid);
fprintf('[export_golden_batch] Done. %d files processed.\n', length(golden.files));
