...
#include "powerGroup/ctrl/ctrl.h"
//#include	"APP_ADC.H"
...
#if 0 /* Moved to powerGroup/drv/drv_isr.c */
...
#endif /* Moved to powerGroup/drv/drv_isr.c */
...
static PowerBase_Input  g_in;
static PowerBase_Output g_out;                                          /* v2.0 Data Switcher 输出槽 */
static const PowerBase_Input *_adc;                                     /* 指向 g_in, Switcher 填入数据后消费 */
...
AppPowerAllDef     PowerAll;			//炉头需要重置频率
...
void	s_pwm_on(void);
/* === Forward declarations — DRV 模块函数 (powerGroup/drv/) === */
...
/* ===== ctrl_StackDelet / ctrl_StackPush — PowerStack 访问封装 ===== */
void ctrl_StackDelet(uint8_t ch)
{
    DataStruct_StackDelet(&PowerAll.stack, ch + 1);
}
void ctrl_StackPush(uint8_t ch)
{
    DataStruct_StackPush(&PowerAll.stack, ch + 1);
}
...
//********检锅部分*********************************************************
	...
	//*****加热部分 (已迁至 ctrl_PowerRun) *************************************** */
		ctrl_SetInputs(chn, CurrentValue, VoltageValue);
		ctrl_SyncHoldFlags(chn, PowerControl->staticReg->flag.bit.PowerHoldMax, PowerControl->staticReg->flag.bit.PowerHoldMin);
		ctrl_PowerRun(chn);
		{
			CtrlPowerState_t cs;
			ctrl_GetState(chn, &cs);
			PowerControl->input->status.ihStatus          = cs.ih_status;
			PowerControl->input->status.targetPowerDiv25  = cs.target_power;
			PowerControl->input->status.actualPowerDiv25  = cs.actual_power;
			PowerControl->input->status.actualPPG         = cs.actual_ppg;
			PowerControl->input->status.powerStatus       = cs.power_status;
			PowerControl->staticReg->PowerDuty            = cs.power_duty;
			PowerControl->staticReg->PPGdutyActual        = cs.duty_actual;
			PowerControl->staticReg->PowerAdcFact         = cs.power_adc_fact;
			PowerControl->staticReg->PowerOffFlag         = cs.off_flag;
			PowerControl->staticReg->PowerLimitFlag       = cs.power_limit_flag;
			PowerControl->staticReg->DeadCnt              = cs.power_dead_cnt;
			PowerControl->staticReg->PpgLimit             = cs.ppg_limit;
			PowerControl->staticReg->ppgLimitMax          = cs.ppg_limit_max;
			PowerControl->staticReg->ppgLimitPower        = cs.ppg_limit_power;
			PowerControl->staticReg->LimitMaxCount        = cs.limit_max_count;
			PowerControl->staticReg->LimitVoltage         = cs.limit_voltage;
			PowerControl->staticReg->limitQ               = cs.limit_Qvalue;
			#undef power_half_adj
			PowerControl->staticReg->power_half_adj       = cs.power_half_adj;
			#undef vcout_delay
			PowerControl->staticReg->powerHalfCnt         = cs.half_cnt;
			PowerControl->staticReg->VcoutDelay           = cs.vcout_delay;
			PowerControl->staticReg->BOTTOMValue          = cs.bottom_value;
			PowerControl->staticReg->TOPValue             = cs.top_value;
			PowerControl->staticReg->flag.bit.powerResume = cs.resume_flag;
			PowerControl->staticReg->flag.bit.Vcout       = cs.vcout_flag;
			PowerControl->staticReg->flag.bit.ppgLock     = cs.ppg_lock_flag;
			PowerControl->staticReg->flag.bit.ppgAdd      = cs.ppg_add_flag;
			PowerAll.minFre = cs.power_min_fre;
			PidReturn[chn]  = cs.pid_return;
		}
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — power_con_fun */
...
/* Moved to powerGroup/ctrl/ctrl_power.c */
#endif
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — get_MAXMIN_PPG */
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — s_power_convert_bass */
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — s_ppg_fun */
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — i_ppg_control */
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — i_ppg_set_limit */
...
#if 0 /* Moved to powerGroup/ctrl/ctrl_power.c — reset_ppg_limit */
void	reset_ppg_limit(void)
{
...
#if 0 /* Moved to powerGroup/drv/drv_pwm.c */
//--------PPGsetDuty--------------------------
...
#endif /* Moved to powerGroup/drv/drv_pwm.c */
...
static MODULE_INPUT_PARAMS(APP_Adc, PowerBase)  s_adcParam[POTNUM];
static MODULE_INPUT_PARAMS(Comm, PowerBase)     s_commParam[POTNUM];
static MODULE_OUTPUT_PARAMS(PowerBase, Drv)     s_drvParam[POTNUM];
...
static void Init(void)
{
    ...
    for (uint8_t i = 0; i < POTNUM; i++) {
        g_in.adc[i].params  = &s_adcParam[i];
        g_in.comm[i].params = &s_commParam[i];
        g_out.head[i].params = &s_drvParam[i];
    }
}
...
static void ProcessInput(void)
{
    ...
    _adc = (const PowerBase_Input *)g_input.para;

    for (ch = 0; ch < POTNUM; ch++) {
        PowerControl = &PowerMem[ch];

#ifdef CurrentFromTxa
        PowerControl->staticReg->current16 = _adc->adc[ch].params->current;
        ...
        PowerControl->input->status.voltageAd = _adc->adc[ch].params->voltage >> 4;
        PowerControl->staticReg->PowerTxaFact = Adc_GetPowerTxa(PotCh1+ch);
        PowerControl->staticReg->phaseValue = _adc->adc[ch].params->phase;
        PowerControl->staticReg->limitQSum = _adc->adc[ch].params->ceilQ;
...
/* === 兼容包装: DRV 迁移过渡 (powerGroup/drv/) ===
 * 原函数 #if 0 在底部, callers 仍用旧名调用。
 * 包装器使用 Power_channel 获取当前通道，转发到新模块。
 */
void s_pwm_off(INT8U off_num)
{
    drv_pwm_off(Power_channel, off_num);
}

void s_pwm_on(void)
{
    drv_pwm_on(Power_channel);
}


/* ============================================================
 *  #if 0 section -- DRV functions moved to powerGroup/drv/
 *  Each function individually wrapped to avoid nesting with
 *  preprocessor directives inside the function body.
 * ============================================================ */

/* ---- drv_fmac ---- */

#if 0 /* Moved to powerGroup/drv/drv_fmac.c */
...
#endif /* API_POWER_PanFmac (closes #if 1) */
#endif /* Moved to powerGroup/drv/drv_fmac.c (closes API_POWER_PanFmac) */
...
#endif /* APP_POWER_PanPluseMessage */
...
#endif /* API_FMAC_AppPowerOverCallBack */
...
#endif /* API_HRTIM1_TEST_CMP1_IRQHandlerCallback */
...
#endif /* API_ADC_Current1AWD_IRQHandlerCallBack */
...
#endif /* API_ADC_Current2AWD_IRQHandlerCallBack */

/* ---- drv_pan_count ---- */

#if 0 /* Moved to powerGroup/drv/drv_pan_count.c */
...
#endif /* APP_POWER_PanCountInitChX */
...
#endif /* APP_POWER_PanCountInitCh1 */
...
#endif /* APP_POWER_PanCountInitCh2 */
...
#endif /* APP_POWER_PanCountInitCh3 */
...
#endif /* APP_POWER_PanCountInitCh4 */
...
#endif /* APP_POWER_PanCountGetValue */
...
#endif /* APP_POWER_PanCountSetValue */
...
#endif /* API_DMA_PAN_IRQHandlerCallBack */
...
#if 0 /* Moved to powerGroup/drv/drv_protect.c */
...
#endif /* APP_POWER_SetTxaAwdValue */
...
#endif /* APP_POWERR_SetTxaAwdValue */
...
#endif /* APP_ADC_getOverAdcChannel */
...
#endif /* API_HRTIM_PanOffCallBack */
