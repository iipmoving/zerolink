...
uint8_t pulse_check_process(uint16_t* data, uint16_t len);
...
#else
    message.array[0].buff = drv_fmac_get_dma_buff();
#endif
    message.array[1].buff = drv_fmac_get_fmac_buff() + 4;
...
void API_POWER_PanCheckPluse(void)
{
...

    {
        ...
        s_pan.pulse_count = pulse_check_process(
            drv_fmac_get_fmac_buff() + FmacLeve / 2,
