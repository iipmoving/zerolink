...
依赖: pyserial, (可选 matplotlib for --plot)
"""
...
class M4DebugApp:
    ...
    def _connect(self):
        ...
        try:
            ...
            messagebox.showerror("连接异常",
                               ...
                               f"请检查是否已安装:\n"
                               f"  python -m pip install pyserial")
        ...
        if ok:
            ...
            self._set_status(f"已连接 {c.port} @ {c.baudrate} baud, 从站={c.slave_addr}, 初始化中...")
            # 后台线程完成初始化，不阻塞 GUI
    ...
    def _post_connect_init(self):
        ...
        # 连接后先关功率, 避免沿用设备旧状态
        self.root.after(0, lambda: self.power_var.set(0))
        client.set_power(0)
        client.set_work_sta(False, fan_on=True)
        client.start_heartbeat(power_watt=0, fan_on=True)

        # 读取遥测
        data = client.read_telemetry()
        ekf = client.read_ekf_telemetry()
        if data:
            self.root.after(0, lambda: self._update_reg_display(data, self.reg_rows))
        if ekf:
        ...
        if not self._init_config:
            raw = client.read_registers(INIT_START_ADDR, INIT_COUNT)
            if raw is not None:
                regs = {}
                for i, (addr, _) in enumerate(INIT_REGS.items()):
                    regs[addr] = raw[i]
                self.root.after(0, lambda r=regs: self._apply_init_regs(r))
        else:
            self.root.after(0, self._refresh_init_display)

        # 启动后台轮询
        self.root.after(0, self._start_monitor)
        self.root.after(0, lambda: self._set_status(
            f"已连接 {client.port} @ {client.baudrate} baud, 从站={client.slave_addr}"))

    def _apply_init_regs(self, regs):
        """主线程: 应用初始寄存器值到 UI"""
        self._init_config.update(regs)
        self._refresh_init_display()
    ...
    def _poll_once(self):
        ...
        try:
            # ====== PM 数据读取（从 MODBUS 客户端提取）======
            pm_frame = self.client.drain_pm() if self.client else []
            if pm_frame:
