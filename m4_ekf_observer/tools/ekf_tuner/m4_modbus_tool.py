...
class M4ModbusClient:
    ...
    def read_raw_capture(self) -> dict | None:
        """读取 0x5100 区原始数据, 帧格式与 WaveCapture 一致 (自描述)

        每帧: [i_adc×N][v_adc×N][cnt×N][cmp_uon..cmp_loff]
          size_prefix = N×3 + 4 (N 采样点数, 4 PP参数)
        ...
        # 1. 读 header (0x5100-0x5105, 6 words)
        hdr = self.read_registers(0x5100, 6)
        if hdr is None:
            return None
        ...
        if count == 0 or data_words == 0:
            self.write_register(0x5100, 1)
            return None

        # 3. 读数据体
        raw = self.read_registers(0x5106, data_words)
        if raw is None:
            return None

        # 4. 按自描述格式解析: [size][frame_data + params]...
        frames = []
        pos = 0
        para_words = 5  # cmp_uon, cmp_uoff, cmp_lon, cmp_loff, power

        for i in range(count):
            if pos >= len(raw):
                break
            fw = raw[pos]       # 本帧总字数 = N×3 + 5
            pos += 1
            if pos + fw > len(raw):
                break
            frame_raw = raw[pos:pos + fw]
            pos += fw
            ...
            i_adc = list(frame_raw[0:n])
            v_adc = list(frame_raw[n:2*n])
            cnt   = list(frame_raw[2*n:3*n])
            cmp_  = list(frame_raw[3*n:3*n + para_words])
