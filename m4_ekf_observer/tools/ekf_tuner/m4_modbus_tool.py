...
class M4ModbusClient:
    ...
    def __init__(self, port: str, baudrate: int = 115200,
        ...
        # MODBUS internal buffer
        self._mb_buf = bytearray()

        # PM state (same semantics as _pm_feed dict)
        self._pm_state = "IDLE"
        self._pm_lines = []
        self._pm_start = 0.0
        self._pm_buf = bytearray()
    ...
    def _read_serial(self) -> bool:
        ...
        Returns True if new data was buffered.
        """
        if not self.ser:
            return False
    ...
    def _check_crc(self, frame: bytes) -> bool:
        ...
        calculated = self._crc16(frame[:-2])
        return received == calculated

    def _extract_modbus_frame(self) -> bytes | None:
        """Scan _mb_buf for a valid MODBUS RTU frame.
        Frame format: [slave(1)][func(1)][data...][crc_lo(1)][crc_hi(1)]
        Returns complete frame bytes (including CRC), or None.
        Consumes matched data from _mb_buf; discards leading non-MODBUS bytes.
        """
        buf = self._mb_buf
        while True:
            if len(buf) < 4:
            ...
            if func & 0x80:
                # MODBUS exception: addr(1) + func|0x80(1) + exc_code(1) + crc(2) = 5
                if len(buf) < 5:
                    return None
                if self._check_crc(buf[:5]):
                    frame = bytes(buf[:5])
                    self._mb_buf = buf[5:]
                    return frame
                buf.pop(0)
            ...
            if func == 0x03:
                ...
                if len(buf) < expected:
                    return None
                if self._check_crc(buf[:expected]):
                    frame = bytes(buf[:expected])
                    self._mb_buf = buf[expected:]
                    return frame
                buf.pop(0)
                continue
    ...
    def connect(self) -> bool:
        """Open serial port. No 'connection' protocol — just open."""
        try:
            import serial
        ...
        print(f"[OK] 已打开 {self.port} @ {self.baudrate} baud, 从站={self.slave_addr}")
        return True
    ...
    def read_registers(self, start_addr: int, count: int) -> list | None:
        """Read holding registers (FC03). Returns list of ints, or None."""
        if not self.ser:
            return None
        with self._lock:
            pdu = bytes([self.slave_addr, 0x03,
                ...
                if mb:
                    ...
                    if func == 0x03:
                        ...
                        for i in range(byte_count // 2):
                            pos = 3 + i * 2
                            registers.append((mb[pos] << 8) | mb[pos + 1])
                        return registers
                    # func=0x83 等错误响应: 设备未就绪, 返回 None
                    return None
                    registers = []
                    for i in range(byte_count // 2):
                        pos = 3 + i * 2
    ...
    def write_register(self, addr: int, value: int) -> bool:
        """Write single register (FC06). Returns True on success."""
        if not self.ser:
            print("[错误] 未连接")
            return False
        with self._lock:
            pdu = bytes([self.slave_addr, 0x06,
                         addr >> 8, addr & 0xFF,
                         value >> 8, value & 0xFF])
            frame = pdu + struct.pack('<H', self._crc16(pdu))
            ...
            while time.monotonic() < deadline:
                self._read_serial()
                if self._extract_modbus_frame():
                    return True

        print(f"[通信错误] 写寄存器 0x{addr:04X}={value}: 无响应")
        return False

    def write_registers(self, addr: int, values: list) -> bool:
        """Write multiple registers (FC10). Returns True on success."""
        if not self.ser:
            print("[错误] 未连接")
            return False
        with self._lock:
            ...
            for v in values:
                data.append(v >> 8)
                data.append(v & 0xFF)
            pdu = bytes([self.slave_addr, 0x10,
                         addr >> 8, addr & 0xFF,
                         count >> 8, count & 0xFF,
                         len(data)]) + data
            frame = pdu + struct.pack('<H', self._crc16(pdu))
        ...
        print(f"[通信错误] 批量写 0x{addr:04X}: 无响应")
        return False

    def drain_pm(self) -> list:
        """Read available serial data and extract PM frames.
        Returns list of row strings from one complete PM frame, or [].
        """
        self._read_serial()

        pm = {
            "state": self._pm_state,
            "lines": self._pm_lines,
            "start": self._pm_start,
            "buf": self._pm_buf,
        }
        result = _pm_feed(pm, b"")

        self._pm_state = pm["state"]
        self._pm_lines = pm["lines"]
        self._pm_start = pm["start"]
        self._pm_buf = pm["buf"]
    ...
    def check_and_init(self, verbose: bool = True,
        ...
        """读取 SYS_STA, 按协议状态机执行:
        - 通信失败或未初始化: 发送 FC10 空写触发初始化, 轮询直到 bit 7=1
        - bit 7 = 1: 设备已就绪, 无需操作
        返回: 设备是否就绪 (可接受功率指令和EKF回读)
        """
        for attempt in range(retry_limit):
            sys_sta = self.get_sys_sta()
            is_init = sys_sta is not None and bool(sys_sta & 0x80)

            if is_init:
                if verbose:
                    print(f"  SYS_STA=0x{sys_sta:02X} (已初始化, 炉头={sys_sta & 0x0F})")
                return True

            # 未初始化或读取失败: 发送 FC10 批写触发 Modbus_I2c_Data_Main()
            if verbose:
                if sys_sta is None:
                    print(f"  [初始化] 通信失败 (尝试 {attempt+1}/{retry_limit}), 发送初始化帧...")
                else:
                    print(f"  SYS_STA=0x{sys_sta:02X} (未初始化, 炉头={sys_sta & 0x0F}), "
                          f"发送初始化帧...")
