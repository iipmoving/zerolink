/**
 * @file    std_module_v2.3.h
 * @brief   统一模块框架 — PULL 路由 + PendSV 实时通路 (v2.3)
 *
 * =====================================================================
 ...
 *
 * 两条通路：
 *   1. 主循环通路：Switcher_Run_Slot1() → 按序调用 DoWork → ProcessInput
 *   2. PendSV 实时通路：ISR → TriggerPendSV → PendSV_Handler → ISRWORK_DoWork
 *
 * 主循环数据流:
 ...
 *   3. name##_OutputCallback(pOut) — weak 空壳, 仅 ST_OUT 时调用, 一般不用
 *
 ...
 *
 * ISR 事件（中断服务）:
 *   紧急低开销: 走独立回调函数，不碰 g_input/g_output
 *   紧急需要数据: ISR 触发 PendSV, PendSV 通路处理
 ...
 *   6. PendSV 通路: 调用 Switcher_RegisterModule() 注册
 * =====================================================================
 *
 * 引用: #include "core/std_module_v2.3.h"
 */

#ifndef STD_MODULE_V2_3_H
#define STD_MODULE_V2_3_H
 ...
 *     - weak void name_OnISR(void)             (ISR 独立入口)
...
#define MODULE_SKELETON(name)                                            \
    ...
    { (void)pOut; }                                                      \
                                                                         \
    __attribute__((weak)) void name##_OnISR(void)                        \
    { }                                                                  \
                                                                         \
 ...
 * 9. PendSV 通路辅助宏 (可选)
 *
 *   仅用于 ISRWORK 这类需要收集多模块数据的模块
 * ==================================================================== */

/* 声明外部 g_output 引用 */
#define DECLARE_MODULE_OUTPUT(name)                                      \
    extern Para_Grp_t g_##name##_output

/* 收集数据宏 (在 CollectInput 中使用) */
#define COLLECT_FROM_MODULE(dst_ptr, src_name, src_type)                 \
    do {                                                                \
        if (g_##src_name##_output.para != NULL) {                       \
            memcpy(dst_ptr, g_##src_name##_output.para, sizeof(src_type)); \
        }                                                               \
    } while (0)
...
/* PendSV Handler (在启动文件中声明，在 pendsv_switcher.c 中实现) */
void PendSV_Handler(void);

/* ISR 处理槽路由 (内部调用，不直接暴露) */
void Switcher_Run_ISR_Slot(uint8_t isr_source);

#endif /* STD_MODULE_V2_3_H */