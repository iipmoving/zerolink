...
//ADC_TypeDef		AdcT12ASave[2];
//ADC_TypeDef		AdcT34ASave[2];
//ADC_TypeDef		AdcVcIcSave[2];

// DMA_Channel_TypeDef	DmaT12ASave[3];
// DMA_Channel_TypeDef	DmaT34ASave[3];
// DMA_Channel_TypeDef	DmaVcIcSave[3];
...
//			API_ADC_InjectedConfig();	//adc2 inj group
		...
		// {	
		// 		API_ADC_VcIc_DISABLE();					//VCIC需要在100US中断中打开
		// }
...
//		API_ADC_DMA_RecoverCallBack(ch);					//实际设置DMA地址 在APP_ADC中实现
...
//		{
//			AdcT12aHandle.Instance=AdcTotalGroup[0].t12a->adc;
//			AdcT34aHandle.Instance=AdcTotalGroup[0].t34a->adc;
//			AdcCurrentHandle.Instance=AdcTotalGroup[0].vcIc->adc;		
//			API_ADC_Instance_RecoverCallBack(0);
//			
//		}
#if 0
	
	
		AdcInstanceSave[ch].VcIcSave.sr=AdcCurrentHandle.Instance->SR;	
		AdcInstanceSave[ch].VcIcSave.cr1=AdcCurrentHandle.Instance->CR1;	
		AdcInstanceSave[ch].VcIcSave.cr2=AdcCurrentHandle.Instance->CR2;
		AdcInstanceSave[ch].VcIcSave.cr3=AdcCurrentHandle.Instance->CR3;
		AdcInstanceSave[ch].VcIcSave.smpr1=AdcCurrentHandle.Instance->SMPR1;		
		AdcInstanceSave[ch].VcIcSave.smpr2=AdcCurrentHandle.Instance->SMPR2;	
		AdcInstanceSave[ch].VcIcSave.sqr1=AdcCurrentHandle.Instance->SQR1;	
		AdcInstanceSave[ch].VcIcSave.sqr2=AdcCurrentHandle.Instance->SQR2;
		AdcInstanceSave[ch].VcIcSave.sqr3=AdcCurrentHandle.Instance->SQR3;	

		AdcInstanceSave[ch].T34ASave.sr=AdcT34aHandle.Instance->SR;	
		AdcInstanceSave[ch].T34ASave.cr1=AdcT34aHandle.Instance->CR1;
		AdcInstanceSave[ch].T34ASave.cr2=AdcT34aHandle.Instance->CR2;
		AdcInstanceSave[ch].T34ASave.cr3=AdcT34aHandle.Instance->CR3;	
		AdcInstanceSave[ch].T34ASave.smpr1=AdcT34aHandle.Instance->SMPR1;			
		AdcInstanceSave[ch].T34ASave.smpr2=AdcT34aHandle.Instance->SMPR2;		
		AdcInstanceSave[ch].T34ASave.sqr1=AdcT34aHandle.Instance->SQR1;		
		AdcInstanceSave[ch].T34ASave.sqr2=AdcT34aHandle.Instance->SQR2;
		AdcInstanceSave[ch].T34ASave.sqr3=AdcT34aHandle.Instance->SQR3;	


		AdcInstanceSave[ch].T12ASave.sr=AdcT12aHandle.Instance->SR;	
		AdcInstanceSave[ch].T12ASave.cr1=AdcT12aHandle.Instance->CR1;
		AdcInstanceSave[ch].T12ASave.cr2=AdcT12aHandle.Instance->CR2;
		AdcInstanceSave[ch].T12ASave.cr3=AdcT12aHandle.Instance->CR3;	
		AdcInstanceSave[ch].T12ASave.smpr1=AdcT12aHandle.Instance->SMPR1;				
		AdcInstanceSave[ch].T12ASave.smpr2=AdcT12aHandle.Instance->SMPR2;			
		AdcInstanceSave[ch].T12ASave.sqr1=AdcT12aHandle.Instance->SQR1;
		AdcInstanceSave[ch].T12ASave.sqr2=AdcT12aHandle.Instance->SQR2;
		AdcInstanceSave[ch].T12ASave.sqr3=AdcT12aHandle.Instance->SQR3;	
		...
		AdcT34ASave[ch].SR=0;
#endif	
//		HAL_ADC_Start(&AdcT12aHandle);
//		HAL_ADC_Start(&AdcT34aHandle);
//		HAL_ADC_Start(&AdcCurrentHandle);		
...
// {
// 	if(HAL_IS_BIT_CLR(AdcCurrentHandle.Instance->CR2, ADC_CR2_JEXTTRIG))
// 	{		
// 		__HAL_ADC_CLEAR_FLAG(&AdcCurrentHandle, ADC_FLAG_EOC);
//   		__HAL_ADC_ENABLE_IT(&AdcCurrentHandle,ADC_IT_EOC);
// 		__HAL_ADC_ENABLE(&AdcCurrentHandle);
// 	}	
// }	
...
// {
// 		__HAL_ADC_CLEAR_FLAG(&AdcCurrentHandle, ADC_FLAG_EOC);
//   		__HAL_ADC_DISABLE_IT(&AdcCurrentHandle,ADC_IT_EOC);
// 		__HAL_ADC_DISABLE(&AdcCurrentHandle);
...
// void 	API_ADC_VcIc_ENABLE_IT_EOC(void)			//返回值代表当前EOC中断是否开启 1：开启 0：关闭
// {
// 	if(HAL_IS_BIT_CLR(AdcCurrentHandle.Instance->CR2, ADC_CR2_JEXTTRIG))
// 	{	
// 		__HAL_ADC_CLEAR_FLAG(&AdcCurrentHandle, ADC_FLAG_EOC);
