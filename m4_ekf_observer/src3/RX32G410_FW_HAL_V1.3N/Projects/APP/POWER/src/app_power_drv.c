/********************************************************************************
    FileName    :  app_power_drv.c
    Author      :  rsl
    Version     :  V1.0.2
    Brief       :  功率设置，调整功能实现 (DRV层封装版本)

    Date        :  2018-10-20
    Modify      :
                   2018-10-20创建
                   2026-06-09 修改为使用 DRV_HRTIM 封装层
...
#include "app_power_constants.h"	// 常量定义（从 API_HRTIM.h 剥离）
//#include	"APP_ADC.H"

/* === HRTIM 函数声明（来自 drv_hrtim.h 和 api_hrtim.h）========= */
...
void API_HRTIM1_TEST_CMP1_IRQHandlerCallback(void)
{
	...
	/* PPG 逼近 — 逐周期比较 PowerDuty vs PPGdutyActual */
	APP_ISR_ApplyPpg();
}
...
void PPGonOffCh1(uint8_t onOff) { (void)onOff; }
void PPGonOffCh2(uint8_t onOff) { (void)onOff; }
void PPGonOffCh3(uint8_t onOff) { (void)onOff; }
void PPGonOffCh4(uint8_t onOff) { (void)onOff; }



void	PPGgetAdcValue_ch1(void);				//	void			(*_PPGgetAdcValue)(void);						//得到对应的ADC值

// PPG 通道函数声明（空函数实现，解除依赖）
...
uint8_t APP_POWER_PPG_BkFlag_Pot1(void);
uint8_t APP_POWER_PPG_BkFlag_Pot2(void);
uint8_t APP_POWER_PPG_BkFlag_Pot3(void);
uint8_t APP_POWER_PPG_BkFlag_Pot4(void);
...
//	PPGcenOnoff,
	...
	APP_POWER_PPG_BkFlag_Pot1,					//得到BKFLAGTimBkFlagCh1,
...
AppPowerFunDef	const 	Power2FunTable=
{
	...
	APP_POWER_PPG_BkFlag_Pot2,					//得到BKFLAGTimBkFlagCh1,
};	
AppPowerFunDef	const 	Power3FunTable=
{
	...
	APP_POWER_PanCountGetValue,			//uint8_t 	(*PanCountGetValue)(void);					//得到检锅数
	APP_POWER_PanCountSetValue,			//void  		(*PanCountSetValue)(uint8_t onOff);					//开启计数器	
	APP_POWER_PPG_BkFlag_Pot3,					//得到BKFLAGTimBkFlagCh1,
};	
...
AppPowerFunDef	const 	Power4FunTable=
{
	...
	APP_POWER_PPG_BkFlag_Pot4,					//得到BKFLAGTimBkFlagCh1,
};	
...
static PowerBase_Input  g_inPara;				// 输入缓存
static PowerBase_Output g_outPara;      //输出缓存                                    /* v2.0 Data Switcher 输出槽 */
...
#if 0				
				overPot=0;
				for(uint8_t i=0;i<POT_NUM_CONST;i++)	
				{
					overPot<<=1;	
...
//	FunPPGsetDuty(PAN_FRE_PWM);
	...
	drv_hrtim_check_pan_pluse(Power_channel);		//PotChWork
...
//			PPGsetHalf(PotCh4,START_FRE_PWM);				 
			...
			drv_hrtim_master_sync_set_period(PowerCycle);
...
void	powerZeroChange(void)
{
	...
	{	
		...
		for(uint8_t i=0;i<POT_MAX_CONST;i++)
		{
...
POWER_CHANGE_CYCLE_Line:
				...
				for(uint8_t i=0;i<POT_NUM_CONST;i++)
				{
					drv_hrtim_ppg_set_value_chx(i,powerCycle[i],powerDuty[i]);
				...
				drv_hrtim_ppg_set_value_chx(PotChTest1,PowerCycle,PowerCycle/2);
				drv_hrtim_ppg_set_value_chx(PotChBase,PowerCycle,PowerCycle/2);
...
{
	drv_hrtim_ppg_on_off(ch,0);				//关PPG
//	APP_ADC_PanSwChange(ch);		//切换PANSW选择合适的ADC通道
	...
	//drv_hrtim_ppg_set_single(Power_channel);	//设置为单脉冲模式
}
...
void	PPGsetHalf(uint8_t ch,uint16_t pwm)
{
	...
	// 空函数调用（后续改为写入输出结构体）
	switch(ch) {
		case PotCh1: PPGsetValueCh1(input); break;
		case PotCh2: PPGsetValueCh2(input); break;
		case PotCh3: PPGsetValueCh3(input); break;
		case PotCh4: PPGsetValueCh4(input); break;
	}
...
void		Power_Adc_Input(void)
{
    ...
    for (uint8_t ch = 0; ch < POTNUM; ch++) {
        ...
        PowerMem[ch].staticReg->current16 = 		g_inPara.adc[ch].params->current;
				PowerMem[ch].input->status.currentAd = PowerControl->staticReg->current16 >> 2;		//为了通讯显示


        PowerMem[ch].input->status.voltageAd = g_inPara.adc[ch].params->voltage>> 4;
        PowerMem[ch].staticReg->PowerTxaFact =	g_inPara.adc[ch].params->power;
        PowerMem[ch].staticReg->phaseValue = g_inPara.adc[ch].params->phase;;
        PowerMem[ch].staticReg->limitQSum = g_inPara.adc[ch].params->current;;
...
void API_POWER_EKF_GetTelemetry(uint8_t chn, EKF_Telemetry_t *ekf)
{
    ...
    // 从空函数获取（后续改为从输入结构体读取）
    switch(chn) {
        case 0: ppg = PPGgetValueCh1(); break;
        case 1: ppg = PPGgetValueCh2(); break;
        case 2: ppg = PPGgetValueCh3(); break;
        case 3: ppg = PPGgetValueCh4(); break;
        default: ppg.duty = 0; ppg.prioed = 0; break;
    }
 ...
 * 输入: g_in (Power_Input_t) ← Switcher 填入 ADC raw
...
static MODULE_INPUT_PARAMS(APP_Adc, PowerBase)  s_adcParam[POWER_POTMAX];
static MODULE_INPUT_PARAMS(Comm, PowerBase)     s_commParam[POWER_POTMAX];
static MODULE_OUTPUT_PARAMS(PowerBase, Drv)     s_drvParam[POWER_POTMAX];

static void Init(void)
{
    g_input.para  = &g_inPara;
    g_output.para = &g_outPara;

    for (uint8_t i = 0; i < POWER_POTMAX; i++) {
        g_inPara.adc[i].params  = &s_adcParam[i];
        g_inPara.comm[i].params = &s_commParam[i];
        g_outPara.head[i].params = &s_drvParam[i];
    }
}
...
/* ===================================================================
 * PPG 操作 — 统一写 PowerMem.staticReg (内存操作)
 *
 * 所有 PPG 输出函数改为写内存，不再直写 HRTIM。
 * ISR (APP_ISR_ApplyPpg) 读取 PowerDuty vs PPGdutyActual，
 * 逐周期逼近 → 写 HRTIM CMP/PER。
 *
 * action: 0=逼近(默认)  1=立即(设 PPGdutyActual=PowerDuty)
 *         2=脉冲(起振检锅, 由检锅状态机处理)
 * =================================================================== */
...
void APP_PPG_Action(uint8_t ch, uint16_t duty, uint16_t cycle, uint8_t action)
{
    ...
    PowerMem[ch].staticReg->PowerDuty = duty;
    if (cycle) {
        PowerMem[ch].staticReg->powerCycle = cycle;
    }

    if (action == 1) {
        /* 立即: ISR 看到相等, 直接写 HRTIM */
        PowerMem[ch].staticReg->PPGdutyActual = duty;
    }
    /* action=2 由检锅状态机处理, 不在此处 */
}

/* ---- Per-channel 写 PowerDuty (setValue = duty + period) ---- */
void PPGsetValueCh1(PPGvalueDef input)
{
    APP_PPG_Action(PotCh1, input.duty, input.prioed, 0);
}
void PPGsetValueCh2(PPGvalueDef input)
{
    APP_PPG_Action(PotCh2, input.duty, input.prioed, 0);
}
void PPGsetValueCh3(PPGvalueDef input)
{
    APP_PPG_Action(PotCh3, input.duty, input.prioed, 0);
}
void PPGsetValueCh4(PPGvalueDef input)
{
    APP_PPG_Action(PotCh4, input.duty, input.prioed, 0);
}

/* ---- Per-channel 读 PowerDuty + powerCycle ---- */
PPGvalueDef PPGgetValueCh1(void)
{
    PPGvalueDef v;
    v.duty   = PowerMem[PotCh1].staticReg->PowerDuty;
    v.prioed = PowerMem[PotCh1].staticReg->powerCycle;
    return v;
}
PPGvalueDef PPGgetValueCh2(void)
{
    PPGvalueDef v;
    v.duty   = PowerMem[PotCh2].staticReg->PowerDuty;
    v.prioed = PowerMem[PotCh2].staticReg->powerCycle;
    return v;
}
PPGvalueDef PPGgetValueCh3(void)
{
    PPGvalueDef v;
    v.duty   = PowerMem[PotCh3].staticReg->PowerDuty;
    v.prioed = PowerMem[PotCh3].staticReg->powerCycle;
    return v;
}
PPGvalueDef PPGgetValueCh4(void)
{
    PPGvalueDef v;
    v.duty   = PowerMem[PotCh4].staticReg->PowerDuty;
    v.prioed = PowerMem[PotCh4].staticReg->powerCycle;
    return v;
}

/* ---- Per-channel 写 PowerDuty (setDuty = 只设占空比, 周期由 PowerCycle 统一管理) ---- */
void PPGsetDutyCh1(uint16_t duty)
{
    APP_PPG_Action(PotCh1, duty, 0, 0);
}
void PPGsetDutyCh2(uint16_t duty)
{
    APP_PPG_Action(PotCh2, duty, 0, 0);
}
void PPGsetDutyCh3(uint16_t duty)
{
    APP_PPG_Action(PotCh3, duty, 0, 0);
}
void PPGsetDutyCh4(uint16_t duty)
{
    APP_PPG_Action(PotCh4, duty, 0, 0);
}

/* ---- Per-channel 写 deadTime (HW 直接操作, 暂存 stub) ---- */
void PPGdeadTimeCh1(uint8_t downDts, uint8_t upDts) { (void)downDts; (void)upDts; }
void PPGdeadTimeCh2(uint8_t downDts, uint8_t upDts) { (void)downDts; (void)upDts; }
void PPGdeadTimeCh3(uint8_t downDts, uint8_t upDts) { (void)downDts; (void)upDts; }
void PPGdeadTimeCh4(uint8_t downDts, uint8_t upDts) { (void)downDts; (void)upDts; }

/* ===================================================================
 * ISR 状态变量 — 仅 APP_ISR_ApplyPpg 使用 (控制路径只写 PowerMem)
 * =================================================================== */
static uint16_t s_pendingMasterPeriod;      /* 待更新的 Master 同步周期 */
static uint8_t  s_onOffLast[POTNUM];         /* 上次写入 HRTIM 的 on/off */

/* ===================================================================
 * ISR 逼近函数 — 每 HRTIM 周期调用
 *
 * 唯一可写 HRTIM 寄存器的地方。控制路径只写 PowerMem.staticReg。
 *
 * 处理:
 *   1. 占空比逼近: PowerDuty(目标) → PPGdutyActual(当前) → 写 CMP
 *   2. 周期同步: powerCycle → 写 PER
 *   3. On/Off 同步: flag.bit.ppgOn → HRTIM on/off
 *   4. Master 周期: s_pendingMasterPeriod → Master PER + Test1/Base
 * =================================================================== */
void APP_ISR_ApplyPpg(void)
{
    for (uint8_t ch = 0; ch < POTNUM; ch++) {
        /* ---- 1. 占空比逼近 ---- */
        uint16_t target = PowerMem[ch].staticReg->PowerDuty;
        uint16_t actual = PowerMem[ch].staticReg->PPGdutyActual;

        if (actual != target) {
            if (actual < target) actual++;
            else                 actual--;
            PowerMem[ch].staticReg->PPGdutyActual = actual;
        }

        /* ---- 2. 写 HRTIM: 周期(PER) + 占空比(CMP) ---- */
        drv_hrtim_ppg_set_value_chx(ch,
            PowerMem[ch].staticReg->powerCycle, actual);

        /* ---- 3. On/Off 同步 ---- */
        uint8_t desired = PowerMem[ch].staticReg->flag.bit.ppgOn ? 1 : 0;
        if (desired != s_onOffLast[ch]) {
            drv_hrtim_ppg_on_off(ch, desired);
            s_onOffLast[ch] = desired;
        }
    }

    /* ---- 4. Master 同步周期更新 (frequency change) ---- */
    if (s_pendingMasterPeriod) {
        drv_hrtim_master_sync_set_period(s_pendingMasterPeriod);
        drv_hrtim_ppg_set_value_chx(PotChTest1,
            s_pendingMasterPeriod, s_pendingMasterPeriod >> 1);
        drv_hrtim_ppg_set_value_chx(PotChBase,
            s_pendingMasterPeriod, s_pendingMasterPeriod >> 1);
        s_pendingMasterPeriod = 0;
    }
}

uint8_t APP_POWER_PPG_BkFlag_Pot1(void) { return 0; }
uint8_t APP_POWER_PPG_BkFlag_Pot2(void) { return 0; }
uint8_t APP_POWER_PPG_BkFlag_Pot3(void) { return 0; }
uint8_t APP_POWER_PPG_BkFlag_Pot4(void) { return 0; }
