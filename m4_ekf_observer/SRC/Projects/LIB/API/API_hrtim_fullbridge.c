...
static const FB_HWMapTypeDef FB_HW_MAP[FB_MAX_CH] = {
    {
        ...
        .masterCmpUnit         = HRTIM_COMPAREUNIT_2,     /* MCMP2 控制FB1移相 */
    ...
    },
    {
        ...
        .masterCmpResetTrigger = HRTIM_TIMRESETTRIGGER_MASTER_CMP3,
        .masterCmpUnit         = HRTIM_COMPAREUNIT_3,     /* MCMP3 控制FB2移相 */
...
void API_FB_Init_MasterSync(uint8_t fbCh)
{
    ...
    /* 先停止计数器，清除半桥残留的运行状态 */
    HAL_HRTIM_WaveformCountStop(&hhrtim1, hw->leadTimerID | hw->lagTimerID);
    ...
    /* ---- 步骤1.5: Master CMP1 = 50% (功率逼近中断触发, HALF模式自动计算) ---- */
...
void API_FB_SinglePulseStart(uint8_t fbCh)
{
    ...
    /* MCMP4 设在周期末尾 -- 此处开/关 OENR */
    __HAL_HRTIM_SETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_MASTER,
                           HRTIM_COMPAREUNIT_4, period - 100U);

    /* 关输出, 复位计数器 */
    g_fb_sp_outPin = outPin;
    g_fb_sp_count  = 0;

    /* 开 CMP4 中断 -- 第一次触发时开 OENR, 第二次关 OENR */
    hhrtim1.Instance->sMasterRegs.MICR |= HRTIM_MICR_MCMP4;
    hhrtim1.Instance->sMasterRegs.MDIER |= HRTIM_MDIER_MCMP4IE;
