...
void				API_PPG_setValue(uint8_t ppgCh,PPGvalueDef value)			//设置PWM输出周期
{
	...
	__HAL_HRTIM_SETPERIOD(&hhrtim1,HRTIM_CFG_NUM[ppgCh].num,value.prioed);
	__HAL_HRTIM_SETCOMPARE(&hhrtim1,HRTIM_CFG_NUM[ppgCh].num,COMPAREUNIT_REST,value.duty);
	__HAL_HRTIM_SETCOMPARE(&hhrtim1,HRTIM_CFG_NUM[ppgCh].num,COMPAREUNIT_BLKS_END,value.duty+BLKS_DIV);		
...
inline void		API_PPG_setPeriod(uint8_t ppgCh ,uint16_t value)	
{
		__HAL_HRTIM_SETPERIOD(&hhrtim1,HRTIM_CFG_NUM[ppgCh].num,value);		
}
...
void	API_FB_PPG_OnOff(uint8_t ppgCh,uint8_t flag)				//PWM输出开始
{
  uint8_t ch=ppgCh*2;     //切成全桥通道
	if(flag)
	{
		API_PPG_ClearFaultFlagCh(ch);			//清除Fault 标志
		API_PPG_ClearFaultFlagCh(ch+1);			//清除Fault 标志
		HrtimOutPutPinSave|=HRTIM_CFG_NUM[ch].outputPin;
		HrtimOutPutPinSave|=HRTIM_CFG_NUM[ch+1].outputPin;
	}
	else
	{
		...
		HrtimOutPutPinSave&=~HRTIM_CFG_NUM[ch].outputPin;
		HrtimOutPutPinSave&=~HRTIM_CFG_NUM[ch+1].outputPin;
		HAL_HRTIM_WaveformOutputStop(&hhrtim1,HRTIM_CFG_NUM[ch].outputPin);
		HAL_HRTIM_WaveformOutputStop(&hhrtim1,HRTIM_CFG_NUM[ch+1].outputPin);
	}
...
// void	API_FB_PPG_OnOff_NoFault(uint8_t ppgCh,uint8_t flag)				//PWM输出开始
#else
  do
  {
      uint8_t ch=ppgCh*2;
	  ...
	  HAL_HRTIM_FaultModeCtl(&hhrtim1,HRTIM_FAULT_NUM[ch].num,  HRTIM_FAULTMODECTL_DISABLED);
	  HAL_HRTIM_FaultModeCtl(&hhrtim1,HRTIM_FAULT_NUM[ch+1].num,  HRTIM_FAULTMODECTL_DISABLED);

	  if(flag)
	  {
	  	HAL_HRTIM_WaveformOutputStart(&hhrtim1,HRTIM_CFG_NUM[ch].outputPin);
	  	HAL_HRTIM_WaveformOutputStart(&hhrtim1,HRTIM_CFG_NUM[ch+1].outputPin);
	  }
	  else
	  {
		  HAL_HRTIM_WaveformOutputStop(&hhrtim1,HRTIM_CFG_NUM[ch].outputPin);
...
/**
 * @brief 检锅单脉冲启动 — HRTIM SimpleOnePulse 模式
 *        输出 1 个激励脉冲后自动停止，替代 API_HRTIM_CHECK_PAN_PLUSE 的连续多脉冲模式
 * @param ch PotCh (1-4)
 * @note  脉冲宽度 = 当前 HRTIM 周期 × 60%
 *        完成后自动触发 HRTIM_PAN 中断 → API_HRTIM_PanOffCallBack
 */
void	API_PPG_SET_SINGLE(uint8_t ch)
{
    static const uint32_t reset_map[] = {
        HRTIM_TIMERRESET_TIMER_B,   /* PotCh1 = TimerB */
        HRTIM_TIMERRESET_TIMER_E,   /* PotCh2 = TimerE */
        HRTIM_TIMERRESET_TIMER_A,   /* PotCh3 = TimerA */
        HRTIM_TIMERRESET_TIMER_D,   /* PotCh4 = TimerD */
    };

    HAL_HRTIM_WaveformCountStop(&hhrtim1, HRTIM_CFG_NUM[ch].TIMERID);

    /* 切换为单次模式 (1个脉冲后自动停止) */
    HAL_HRTIM_TimeBaseConfig(&hhrtim1, HRTIM_CFG_NUM[ch].num,
        (HRTIM_TimeBaseCfgTypeDef *)&PPGSingleTimeBaseCfg);
    HAL_HRTIM_SoftwareUpdate(&hhrtim1, HRTIM_CFG_NUM[ch].num);
    ...
    /* 使能输出 + 启动计数 → 软件复位触发计数 (单次模式需复位事件才开始) */
 
    HAL_HRTIM_WaveformCountStart(&hhrtim1, HRTIM_CFG_NUM[ch].TIMERID);
   HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_CFG_NUM[ch].outputPin);		
    HAL_HRTIM_SoftwareReset(&hhrtim1, reset_map[ch]);  /* TxRST=1 → 开始计数 */

    /* 延时等待脉冲完成, 然后停止定时器(否则输出停留在最后状态) */
    volatile uint32_t dly = 200; while (dly--);
   HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_CFG_NUM[ch].outputPin);	
}
...
{
    /* HALF模式: 写MPER时硬件自动设MCMP1=MPER/2 (50%), 无需手动写CMP1 */
    hhrtim1.Instance->sMasterRegs.MCR |= HRTIM_MCR_HALF;
    ...
    // MASTER CMP1 = 50% (PPG 步进触发)
    HRTIM_CompareCfgTypeDef cmpCfg = {0};
    cmpCfg.CompareValue = (uint16_t)((uint32_t)masterPeriod * 50 / 100);
    HAL_HRTIM_WaveformCompareConfig(&hhrtim1, HRTIM_TIMERINDEX_MASTER, HRTIM_COMPAREUNIT_1, &cmpCfg);
...
void API_HRTIM_MasterSync_ConfigSlave(uint8_t ch, uint16_t slavePeriod)
{
    ...
    HAL_HRTIM_TimeBaseConfig(&hhrtim1, timerIdx, &timeBaseCfg);

    /* ---- CMP4(占空比) + CMP3(消隐/消抖) 按比例更新 ---- */
    __HAL_HRTIM_SETCOMPARE(&hhrtim1, timerIdx, COMPAREUNIT_REST, newDuty);
    __HAL_HRTIM_SETCOMPARE(&hhrtim1, timerIdx, COMPAREUNIT_BLKS_END, newDuty + BLKS_DIV);

    /* ---- 定时器同步配置 ---- */
    HRTIM_TimerCfgTypeDef timerCfg = PPGTimerCfg;
    timerCfg.FaultEnable           = HRTIM_CFG_NUM[ch].FaultEnable;
    timerCfg.DelayedProtectionMode = HRTIM_CFG_NUM[ch].DelayedProtectionMode;
    ...
    HAL_HRTIM_WaveformTimerConfig(&hhrtim1, timerIdx, &timerCfg);
    HAL_HRTIM_SoftwareUpdate(&hhrtim1, timerIdx);
}
...
void API_HRTIM_MasterSync_StartAll(void)
{
    ...
    __HAL_HRTIM_SETCOUNTER(&hhrtim1, HRTIM_TIMERINDEX_TIMER_F, 0);
			...
			hhrtim1.Instance->sMasterRegs.MCR |= HRTIM_MCR_PREEN|HRTIM_MCR_MREPU;		
...
{
    uint32_t misr = hhrtim1.Instance->sMasterRegs.MISR;
    uint32_t mier = hhrtim1.Instance->sMasterRegs.MDIER;
    uint32_t active = misr & mier;

    if (active & HRTIM_MISR_MCMP1) {
        /* CMP1@50%: per-cycle stepping trigger (continuous) */
        __HAL_HRTIM_MASTER_CLEAR_FLAG(&hhrtim1, HRTIM_MISR_MCMP1);
        API_HRTIM1_TEST_UPD_IRQHandlerCallback(1);
    }
    if (active & HRTIM_MISR_MUPD) {
...
//        API_HRTIM1_TEST_UPD_IRQHandlerCallback(1);
    ...
    if (active & HRTIM_MISR_MCMP4) {
        /* CMP4: 对角单脉冲控制 (开/关 OENR) */
        __HAL_HRTIM_MASTER_CLEAR_FLAG(&hhrtim1, HRTIM_MISR_MCMP4);
...
{
    __HAL_HRTIM_MASTER_CLEAR_FLAG(&hhrtim1, HRTIM_MISR_MUPD);
    __HAL_HRTIM_MASTER_ENABLE_IT(&hhrtim1, HRTIM_MASTER_IT_MUPD);
    HAL_NVIC_SetPriority(HRTIM1_Master_IRQn, IRQ_PRIORITY_HRTIM_Master, 1);
    HAL_NVIC_EnableIRQ(HRTIM1_Master_IRQn);
}

void API_HRTIM_Master_DISABLE_IT_UPD(void)
{
    __HAL_HRTIM_MASTER_DISABLE_IT(&hhrtim1, HRTIM_MASTER_IT_MUPD);
}

void API_HRTIM_Master_ENABLE_IT_CMP1(void)
{
    __HAL_HRTIM_MASTER_CLEAR_FLAG(&hhrtim1, HRTIM_MISR_MCMP1);
    __HAL_HRTIM_MASTER_ENABLE_IT(&hhrtim1, HRTIM_MASTER_IT_MCMP1);
    HAL_NVIC_SetPriority(HRTIM1_Master_IRQn, IRQ_PRIORITY_HRTIM_Master, 1);
    HAL_NVIC_EnableIRQ(HRTIM1_Master_IRQn);
}

void API_HRTIM_Master_DISABLE_IT_CMP1(void)
{
